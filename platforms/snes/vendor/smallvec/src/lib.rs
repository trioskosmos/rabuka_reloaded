...
// The sizes below (>= 0x10000, all multiples of 65536) overflow a 16-bit
// `usize` and wrap to 0, colliding with the `[T; 0]` impl above. Exclude them
// on 16-bit-pointer targets (e.g. the SNES/65816 via rust-mos). On 32/64-bit
// targets these are unchanged, so behavior is identical everywhere else.
#[cfg(all(not(feature = "const_generics"), not(target_pointer_width = "16")))]
impl_array!(
    0x10000, 0x20000, 0x40000, 0x60000, 0x80000, 0x10_0000
);
