...
powerpc-eabi-gcc -mrvl -meabi -mhard-float -I"%DEVKITPRO%\libogc\include" -o "%~dp0output\rabuka_wii.elf" entry.c -L"C:\rust_targets\powerpc-unknown-eabi\release" -lrabuka_wii -L"%DEVKITPRO%\libogc\lib\wii" -logc -lc -lm
if %ERRORLEVEL% neq 0 ( echo FAIL; pause; exit /b 1 )

echo [3/5] DOL...
elf2dol "%~dp0output\rabuka_wii.elf" "%~dp0output\rabuka_wii.dol"
echo DONE %~dp0output\rabuka_wii.dol
pause