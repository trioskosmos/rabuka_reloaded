...
/// 16x16 (2x2 tiles, 12px font) 窶・each text line is 2 tile-rows.
...
/// mirrored by `tools/bake_texticon_tiles.py` PALETTE_TARGETS 窶・keep in sync.
...
impl<'a> Display<'a> {
    ...
    pub fn render_board_frame(&mut self, frame: &BoardFrame) {
        ...
        if self.detail_active {
            // Detail view overwrote 0-239 with per-card palette 窶・restore master
            for i in 0..240 {
                let v = MASTER_PAL[i * 2] as u16 | ((MASTER_PAL[i * 2 + 1] as u16) << 8);
        ...
        // DETAIL_TEXT_PALETTE 窶・restore it
        ...
        // Stage rows: opponent (flipped 180ﾂｰ) then player, with live success + live set stacked on right
        for (row, y) in STAGE_YS.iter().enumerate() {
            ...
            // Live card set zone (bottom 3 rows of stage row) 窶・flipped like 3DS landscape handling
...
for (i, slot) in frame.hand.iter().enumerate() {
    ...
    /// 17 columns. `scroll` is the first visible line index 窶・paginated like
    ...
    /// previous screen and the new screen briefly coexist 窶・a 280-tile
        ...
        for q in self.pending_art.drain(..) {
                ...
                .find(|f| f.card_no == q.card_no.as_str())
            {
                ...
                if q.dimmed {
                    ...
                    // fill over the art 窶・the tiled equivalent of the 3DS
    ...
    fn draw_slot(
        ...
        if slot.actionable {
            // Gold badge on the card itself (top-right corner, or bottom-left when flipped 180ﾂｰ)
