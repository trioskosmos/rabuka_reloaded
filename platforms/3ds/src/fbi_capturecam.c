...
#ifndef R_APP_OUT_OF_MEMORY
#define R_APP_OUT_OF_MEMORY 0xDEAD0001
#endif
#ifndef R_APP_INVALID_ARGUMENT
#define R_APP_INVALID_ARGUMENT 0xDEAD0002
#endif
#ifndef R_APP_THREAD_CREATE_FAILED
#define R_APP_THREAD_CREATE_FAILED 0xDEAD0003
#endif
...
Result task_capture_cam(capture_cam_data* data) {
    ...
    if(R_FAILED(res)) {
        data->finished = true;
        if(data->cancelEvent != 0) { svcCloseHandle(data->cancelEvent); data->cancelEvent = 0; }
        if(data->mutex != 0) { svcCloseHandle(data->mutex); data->mutex = 0; }
    }
