...
/* Jaguar port: no TLS (freestanding single-threaded m68k). TLS on every
   wasm2c function prologue forces GOT/TLS relocations we can't link. */
...
/* Jaguar port: with WASM_RT_INSTALL_SIGNAL_HANDLER=0 use plain setjmp/longjmp
   everywhere (provided by jag_setjmp.S; no sigsetjmp in this freestanding env). */
