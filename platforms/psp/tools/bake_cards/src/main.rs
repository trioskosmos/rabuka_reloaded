...
fn main() {
    ...
    for (cn, card) in &unique_cards {
        ...
        let base_heart_u8 = match card.base_heart {
            rabuka_engine::card::HeartColor::Red => 1u8,
            rabuka_engine::card::HeartColor::Blue => 2u8,
            rabuka_engine::card::HeartColor::Green => 3u8,
            rabuka_engine::card::HeartColor::Yellow => 4u8,
            rabuka_engine::card::HeartColor::Purple => 5u8,
        };
        let blade_heart_u8 = match card.blade_heart {
            Some(rabuka_engine::card::HeartColor::Red) => 1u8,
            Some(rabuka_engine::card::HeartColor::Blue) => 2u8,
            Some(rabuka_engine::card::HeartColor::Green) => 3u8,
            Some(rabuka_engine::card::HeartColor::Yellow) => 4u8,
            Some(rabuka_engine::card::HeartColor::Purple) => 5u8,
            _ => 0u8,
        };
        ...
        rec.push(card.group);
        rec.push(card.series);
        rec.push(card.unit);
        rec.push(blade_heart_u8);
        rec.push(card.special_heart as u8);
    ...
    // str_table_size placeholder
    let str_size_offset = bin.len();
    ...
    let str_table_offset = bin.len();
    bin.extend_from_slice(&str_bytes);
    // patch str_table_size
    bin[str_size_offset..str_size_offset + 4]
        .copy_from_slice(&(str_bytes.len() as u32).to_le_bytes());
    ...
    for i in 0..deck_entries.len() {
        ...
        bin.extend_from_slice(&name_off.to_le_bytes());
        bin.extend_from_slice(&(card_nos.len() as u16).to_le_bytes());
        for cn in &card_nos {
    ...
    // Rebuild with final string table (offsets may have shifted)
    // Actually str_offsets were built incrementally, so they're correct.
    // But we need to rebuild because deck index calls add_str after card records.
    // Just write what we have — the str_offsets are already correct from the first pass.
