...
if not exist "%~dp0..\psp\baked\decks.json" (
...
if exist "%~dp0output\disc\1ST_READ.BIN" (
    echo.
    echo ========================================
    echo  Dreamcast build successful!
    echo  ELF: output\rabuka_dc.elf
    echo  1ST_READ.BIN: output\disc\1ST_READ.BIN
    echo ========================================
    echo.
    echo To create a CD image, use:
    echo   wsl -d Ubuntu sh-elf-objcopy ... (or use a tool like cdirip)
) else (
    if exist "%~dp0output\rabuka_dc.elf" (
