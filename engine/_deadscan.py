...
bases = [
    ...
    os.path.join(root,'platforms','3ds','src'),
    os.path.join(root,'platforms','gba','src'),
]
        ...
        for fn in files:
            if fn.endswith('.rs'):
                ...
                try:
                    ...
                    continue
                corpus.append(s)
                if 'engine\\src' in p or 'engine/src' in p:
...
open(os.path.join(root,'engine','_dead.txt'),'w').write('\n'.join(out))
