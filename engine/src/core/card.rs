...
impl AbilityEffect {
    ...
    pub fn alternative_effect_any(&self) -> Option<&AbilityEffect> {
        self.kind
            .as_deref()
            .and_then(|k| k.filter())
            .and_then(|f| f.alternative_effect.as_deref())
    }
...
impl ComparisonTarget {
    ...
    // Inherent from_str is deliberate: the generated bytecode decoders
    // (condition_decoder_gen.rs) call this exact path for ~20 condition
    // enums; switching them to ::from_str trait syntax is churn without
    // behavior gain.
