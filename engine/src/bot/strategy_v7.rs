//! Strategy bot v7 — matches v6's win rate by using v6's pass logic.
//!
...
pub fn choose_action_v7(gs: &GameState, actions: &[Action], me: u8) -> Action {
    ...
    // v7 fix: match v6's pass logic exactly (pass on neutral/negative), keep baton scoring bonus
    ...
    for (i, a) in actions.iter().enumerate() {
        if a.action_type == ActionType::Pass {
            // v6 logic: Pass = -inf if any play > 0, else Pass = 0 (pass on neutral/negative)
