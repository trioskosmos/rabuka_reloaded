...
/// 1-ply search using Q(s,a) = value head: evaluate each action individually.
pub fn search_1ply(
    obs: &PublicObservation,
    actions: &[Action],
    _config: &super::BotConfig,
    _sampler: &DeterminizationSampler,
    network: &PolicyNet,
) -> usize {
    ...
    for (i, action) in actions.iter().enumerate() {
        ...
        let act_card = action
            ...
            .unwrap_or(0);
        let q_val = network.q_value(&my_cards, &opp_cards, act_type, act_card) as f64;
        if q_val > best_score {
            best_score = q_val;
            best_idx = i;
        }
