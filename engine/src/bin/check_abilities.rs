...
fn main() {
    ...
    for card in &cards {
        for ability in &card.abilities {
            ...
            if let Some(ref effect) = ability.effect {
                if effect.compound.look_action.is_some() {
                    complexity += 2;
                }
                if effect.compound.select_action.is_some() {
                    complexity += 2;
                }
                ...
                if effect.compound.primary_effect.is_some() {
                    complexity += 2;
                }
                if effect.compound.alternative_condition.is_some() {
                    complexity += 2;
                }
            ...
            if let Some(ref cost) = ability.cost {
                if cost.action == "sequential_cost" {
    ...
    println!(
        "Most complex ability (complexity {}): {}",
        max_complexity, most_complex_name
    );
