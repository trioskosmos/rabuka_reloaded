...
fn decode_condition_field(
    ...
    match key {
            ...
            "blade_greater_than_all" => { l.blade_greater_than_all = bc.read_bool_value(); return Some(true); }
            "blade_limit" => { l.blade_limit = bc.read_u8_value(); return Some(true); }
            "blade_limit_operator" => { l.blade_limit_operator = bc.read_operator_value(); return Some(true); }
            "cache" => { l.cache = bc.read_bool_value(); return Some(true); }
            "card_names" => { l.card_names = bc.read_opt_str_vec_value(); return Some(true); }
            ...
            "check_self" => { l.check_self = bc.read_bool_value(); return Some(true); }
            ...
            "original_value" => { l.original_value = bc.read_bool_value(); return Some(true); }
            ...
            "reference_card" => { l.reference_card = bc.read_arc_str_value(); return Some(true); }
            ...
            _ => { note_decode_fallback(Some(bc.idx.unwrap_or(usize::MAX)), "condition_field", key); bc.skip_value()?; return Some(true); }
        }
    }
