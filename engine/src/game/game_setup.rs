...
fn make_action(action_type: ActionType, description: &str) -> Action {
    Action { description: description.to_string(), action_type, parameters: None }
}

fn make_action_params(action_type: ActionType, description: &str, params: ActionParameters) -> Action {
    Action { description: description.to_string(), action_type, parameters: Some(params) }
}
...
pub fn generate_possible_actions(game_state: &GameState) -> Vec<Action> {
    ...
    match game_state.current_phase {
        ...
        crate::game_state::Phase::ChooseFirstAttacker => generate_choose_first_attacker_actions(game_state),
        crate::game_state::Phase::MulliganP1Turn | crate::game_state::Phase::MulliganP2Turn => generate_mulligan_actions(game_state),
        crate::game_state::Phase::Active | crate::game_state::Phase::Energy | crate::game_state::Phase::Draw => Vec::new(),
        crate::game_state::Phase::Main => generate_main_phase_actions(game_state),
        crate::game_state::Phase::LiveCardSetP1Turn | crate::game_state::Phase::LiveCardSetP2Turn => generate_live_card_set_actions(game_state),
        crate::game_state::Phase::FirstAttackerPerformance | crate::game_state::Phase::SecondAttackerPerformance
    ...
    match choice {
        Choice::SelectTarget { target, description, .. } => {
            ...
            if target == "choice" {
                return description.split(" / ").enumerate().map(|(i, opt)|
                    make_action_params(ActionType::ChoiceOption, opt, ActionParameters { card_id: Some(i as i16), card_no: Some(i.to_string()), ..make_params() })
                ).collect();
            }
        ...
        Choice::SelectCard { zone, card_type, count: _, description, allow_skip, .. } => {
            ...
            let card_ids: Vec<(usize, i16)> = match zone.as_str() {
                ...
                "revealed_cards" => game_state.revealed_cards.iter().copied().enumerate().map(|(i, id)| (i, id)).collect(),
                _ => Vec::new(),
            };
            if !card_ids.is_empty() {
                for (zone_index, card_id) in &card_ids {
                    ...
                    actions.push(make_action_params(ActionType::ChoiceSelect, &format!("{} ({})", card_name, zone_index),
                        ActionParameters { card_id: Some(*card_id), card_index: Some(*zone_index), card_indices: Some(vec![*zone_index]), card_name: Some(card_name.to_string()), card_no: Some("select".to_string()), ..make_params() }));
                }
            } else {
                actions.push(make_action_params(ActionType::ChoiceSelect, &format!("Select card(s): {}", description),
                    ActionParameters { card_indices: Some(Vec::new()), card_no: Some("select".to_string()), ..make_params() }));
            }
            if *allow_skip {
                actions.push(make_action_params(ActionType::ChoiceSkip, "Skip", ActionParameters { card_no: Some("skip".to_string()), ..make_params() }));
            }
            actions
        }
        Choice::SelectPosition { position, description, .. } => {
            position.split(',').map(|a| a.trim()).map(|area| {
                ...
                make_action_params(ActionType::ChoicePosition, &format!("Place at {}: {}", area, description),
                    ActionParameters { stage_area: stage_area_str, card_no: Some("select".to_string()), ..make_params() })
            }).collect()
        }
...
fn generate_mulligan_actions(game_state: &GameState) -> Vec<Action> {
    ...
    let mut actions = vec![make_action(ActionType::MulliganHeader, &format!("{}'s Mulligan Phase", player_name))];
    ...
    for (hand_index, card_id) in mulligan_player.hand.cards.iter().enumerate() {
        ...
        let card_name = game_state.card_database.get_card(*card_id).map(|c| c.name.as_str()).unwrap_or("Unknown");
        actions.push(make_action_params(ActionType::SelectMulligan,
            &format!("{} {} for mulligan", if is_selected { "Deselect" } else { "Select" }, card_name),
            ActionParameters { card_id: Some(*card_id), card_index: Some(hand_index), stage_area: Some("left".to_string()), ..make_params() }));
    }

    actions.push(make_action_params(ActionType::ConfirmMulligan, &format!("Confirm {}'s mulligan", player_name),
        ActionParameters { card_indices: Some(vec![]), ..make_params() }));
    actions.push(make_action(ActionType::SkipMulligan, &format!("Skip {}'s mulligan (keep all cards)", player_name)));
    actions
}
        ...
        for (hand_index, card_id) in active_player.hand.cards.iter().enumerate() {
            if let Some(card) = game_state.card_database.get_card(*card_id) {
                if card.is_member() && !card.is_live() {
                    ...
                    let stage_card_ids = [active_player.stage.stage[0], active_player.stage.stage[1], active_player.stage.stage[2]];

                    for (area_idx, (area, area_name)) in areas.iter().enumerate() {
                        let mut area_info = AreaInfo {
                            area: area_name.to_string(), available: false, cost: card_cost,
                            is_baton_touch: false, existing_member_name: None,
                        };
                    ...
                    if has_any_available {
                        let cost_details: Vec<String> = available_areas.iter().filter(|a| a.available).map(|a| {
                            let name = match a.area.as_str() { "left" => "Left", "center" => "Center", "right" => "Right", o => o };
                            if a.is_baton_touch { format!("{}: {} (baton touch from {})", name, a.cost, a.existing_member_name.as_deref().unwrap_or("existing")) }
                            else { format!("{}: {}", name, a.cost) }
                        }).collect();
                        let cost_str = if cost_details.is_empty() { format!("Cost: {}", card_cost) } else { format!("Cost: {}", cost_details.join(", ")) };

                        actions.push(make_action_params(ActionType::PlayMemberToStage, &format!("{} ({}) - {}", card.name, card.card_no, cost_str),
                            ActionParameters {
                                card_id: Some(*card_id), card_index: Some(hand_index),
                                card_name: Some(card.name.clone()), card_no: Some(card.card_no.clone()),
                                base_cost: Some(card_cost), available_areas: Some(available_areas), ..make_params()
                            }));
                    }
                }
            }
    ...
    for (card_id, area_name) in stage_positions {
        ...
        if let Some(card) = game_state.card_database.get_card(card_id) {
            ...
            for (ability_index, ability) in card.abilities.iter().enumerate() {
                ...
                if !can_activate { continue; }
                if !crate::zones::check_trigger_position(ability.triggers.as_deref(), card_position) { continue; }
                if !crate::zones::check_effect_position(ability.effect.as_ref().and_then(|e| e.activation_position.as_deref()), card_position) { continue; }

                let ability_key = format!("{}_{}_{}", card_id, ability_index, game_state.turn_number);
                if ability.use_limit.is_some() && game_state.turn_limited_abilities_used.contains(&ability_key) { continue; }

                let ability_name = if ability.full_text.chars().count() > 30 {
                    format!("{}...", ability.full_text.chars().take(30).collect::<String>())
                } else { ability.full_text.clone() };
                ...
                actions.push(make_action_params(ActionType::UseAbility,
                    &format!("Use ability on {} ({}): {}{} - Cost: {}", card.name, area_name, ability_name, trigger_info, ability_cost),
                    ActionParameters {
                        card_id: Some(card_id), stage_area: Some(area_name.to_string()),
                        card_name: Some(card.name.clone()), card_no: Some(card.card_no.clone()),
                        base_cost: Some(ability_cost), final_cost: Some(ability_cost), ..make_params()
                    }));
            }
        }
    }
...
fn generate_live_card_set_actions(game_state: &GameState) -> Vec<Action> {
    let active_player = game_state.active_player();
    let mut actions = vec![make_action(ActionType::Pass, "Pass - Finished setting live cards")];

    let can_add_more = active_player.live_card_zone.cards.len() < 3;
    if can_add_more && !game_state.is_action_prohibited("cannot_live") {
        for (hand_index, card_id) in active_player.hand.cards.iter().enumerate() {
            let card_name = game_state.card_database.get_card(*card_id).map(|c| c.name.as_str()).unwrap_or("Unknown");
            actions.push(make_action_params(ActionType::SetLiveCard, &format!("Place {} to live zone", card_name),
                ActionParameters { card_id: Some(*card_id), card_index: Some(hand_index), card_indices: Some(vec![hand_index]), ..make_params() }));
        }
    }
    actions
}
