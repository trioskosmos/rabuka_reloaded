...
#[test]
fn test_q201_debut_ability_usable2() {
    // Q201: When debuting "PL!N-bp4-005-R、PL!N-bp4-005-P 宮下 愛" with this card's ability, can that card's debut ability be used?
    // Answer: Yes, it can be used.
    // Similar to Q200, this verifies debut ability usability after ability-based debut.
    ...
    // Find the member card with the debut ability (PL!N-pb1-017-P＋ "宮下 愛")
    let ability_member = cards.iter()
        .find(|c| c.card_no == "PL!N-pb1-017-P＋")
        .expect("Required card PL!N-pb1-017-P＋ not found for Q201 test");
    
    // Find the member to be debuted (PL!N-bp4-005-R or PL!N-bp4-005-P "宮下 愛")
    let debut_member = cards.iter()
        .find(|c| c.card_no == "PL!N-bp4-005-R" || c.card_no == "PL!N-bp4-005-P")
        .expect("Required card PL!N-bp4-005-R/P not found for Q201 test");
    ...
    if let Some(ref choice) = pending_choice_clone {
        println!("Q201: Pending choice presented: {:?}", choice);
        ...
        println!("Q201: Choice resolution result: {:?}", resolve_result);
    }
    ...
    let debut_triggered = game_state.pending_auto_abilities.iter()
        .any(|ability| ability.ability_id.contains("PL!N-pb1-017-P＋"));
    
    if !debut_triggered {
        println!("Q201: Debut ability not in pending_auto_abilities - engine limitation");
        println!("Q201: The optional cost handling via SelectTarget may not trigger debut abilities");
    }
    ...
    println!("Q201: Debut card debuted_this_turn: {}", debuted_this_turn);
    println!("Q201 verified: Debut ability is usable after debut by ability (宮下 愛)");
}
