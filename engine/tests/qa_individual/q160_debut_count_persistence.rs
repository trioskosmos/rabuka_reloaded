...
fn test_q160_debut_count_persistence() {
    ...
    if let Some(member) = member_card {
        ...
        assert_eq!(game_state.player1.hand.cards.len(), 2, "Hand should have 2 cards");
        
        // Simulate 3 members debuting this turn
        game_state.player1.debuted_this_turn = vec![1, 2, 3];
        
        // One of the debuted members leaves the stage (moves to discard)
        game_state.player1.waitroom.cards.push(1);
        
        // Verify debut count is still 3 (members that left stage still count)
        assert_eq!(game_state.player1.debuted_this_turn.len(), 3, "Debut count should still be 3");
        ...
        let current_hand_size = game_state.player1.hand.cards.len();
