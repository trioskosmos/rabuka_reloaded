...
fn test_q168_multiplayer_partial() {
    ...
    // Find the member card with this ability (PL!-pb1-018-R "E)
    ...
    if let Some(member) = member_card {
        ...
        if let Some(cost2) = cost2_member {
            ...
            player1.waitroom.cards.push(cost2_id);
            ...
            game_state.player1.waitroom.cards.push(cost2_id);
            
            // Verify player1 has cost 2 member in discard
            let player1_has_cost2 = game_state.player1.waitroom.cards.iter()
                .filter(|&&id| {
            ...
            let player2_has_cost2 = game_state.player2.waitroom.cards.iter()
                .filter(|&&id| {
                    if let Some(card) = game_state.card_database.get_card(id) {
                        card.is_member() && card.cost.unwrap_or(0) <= 2
                    } else {
