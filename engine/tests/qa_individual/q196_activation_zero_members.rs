...
#[test]
fn test_q196_activation_zero_members() {
    // Q196: Can you use an activation ability when you have 0 members on stage?
    ...
    // Find a member card with activation ability
    ...
    if let Some(card) = activation_card {
        ...
        // Verify no members on stage
        assert!(game_state.player1.stage.stage.is_empty(),
            "Player should have no members on stage");
        ...
        // The key point: activation abilities can be used even with 0 members on stage
        // This test verifies the card can be played when stage is empty
        assert!(game_state.player1.stage.stage.contains(&card_id),
            "Card should be on stage after playing");
        
        println!("Activation ability can be used with 0 members on stage");
    } else {
