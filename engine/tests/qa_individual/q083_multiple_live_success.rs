...
fn test_q083_multiple_live_success() {
    ...
    if live_cards.len() >= 2 {
        ...
        player1.live_card_zone.push(live1_id);
        player1.live_card_zone.push(live2_id);
        ...
        assert_eq!(game_state.player1.live_card_zone.len(), 2, "Should have 2 live cards in live card zone");
        
        // Simulate live victory with multiple live cards
        // Player chooses 1 to put in success live card zone
        let chosen_live = live1_id;
        game_state.player1.success_live_zone.push(chosen_live);
        game_state.player1.live_card_zone.retain(|&id| id != chosen_live);
        
        // Verify only 1 live card went to success zone
        assert_eq!(game_state.player1.success_live_zone.len(), 1, "Should have 1 live card in success zone");
        assert_eq!(game_state.player1.live_card_zone.len(), 1, "Should have 1 live card remaining in live card zone");
