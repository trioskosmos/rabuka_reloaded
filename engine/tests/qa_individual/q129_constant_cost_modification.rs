...
fn test_q129_constant_cost_modification() {
    ...
    // Question: If hand has 5 cards including "渡辺 曜&鬼塚夏美&大沢瑠璃乃" (which has constant ability: cost is -1 for each other card in hand), and you reveal that card, do you gain the +1 live total score ability?
    ...
    // Find the member card with this activation ability (PL!SP-pb1-003-R+ "嵐 千砂都")
    let activation_member = cards.iter()
        .find(|c| c.card_no == "PL!SP-pb1-003-R+");
    ...
    if let (Some(activation), Some(combined)) = (activation_member, combined_card) {
        ...
        assert_eq!(game_state.player1.hand.len(), 5, "Hand should have 5 cards");
        ...
        let other_cards_count = game_state.player1.hand.len() - 1; // Exclude the combined card itself
