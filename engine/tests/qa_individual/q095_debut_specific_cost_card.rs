...
fn test_q095_debut_specific_cost_card() {
    // Q95: Debut ability (cost: send 1 Liella! member except "E" from stage to discard) - debut 1 copy of that card from discard to that area
    ...
    let member_card = cards.iter()
        ...
        .expect("Required card PL!SP-pb1-011-R not found for Q095 test");
    ...
    let liella_member = cards.iter()
        ...
        .next()
        .expect("Required Liella! member not found for Q095 test");
    
    let liella_id = get_card_id(liella_member, &card_database);
    ...
    player1.waitroom.cards.push(liella_id); // Same card in discard
    ...
    game_state.player1.waitroom.cards.push(liella_id);
    game_state.player1.stage.stage[1] = -1;
    
    // Verify Liella member is in discard
    assert!(game_state.player1.waitroom.cards.contains(&liella_id), "Liella member should be in discard");
