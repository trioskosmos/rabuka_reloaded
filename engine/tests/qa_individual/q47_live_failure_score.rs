...
#[test]
fn test_q47_live_failure_score() {
    // Q47: Does total score become 0 if live fails?
    // Answer: No, it becomes a state of having no total score. For example,
    // if A succeeds at live and B fails, when comparing total scores,
    // A's score is treated as higher than B's regardless of A's score value.
    ...
    if let Some(card) = member_card {
        ...
        // Add live card to success live card zone (simulating successful live)
        ...
        // The key point: failed live doesn't have total score of 0, but has no total score state
