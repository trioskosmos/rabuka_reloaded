...
#[test]
fn test_q218_constant_ability_baton_touch() {
    // Q218: When baton touching a member card without abilities from hand to debut with this card, does this card's constant ability apply?
    ...
    let member_card = cards.iter()
        ...
        .expect("Required card PL!S-bp5-001-R＋ not found for Q218 test");
    
    let member_id = get_card_id(member_card, &card_database);
    
    // Add a member card without abilities to hand for baton touch
    let no_ability_member: Vec<_> = cards.iter()
    ...
    assert!(!no_ability_member.is_empty(), "Need at least 1 other member for Q218 test");
    let hand_member_id = get_card_id(no_ability_member[0], &card_database);
    assert!(hand_member_id != 0, "Hand member ID is invalid (0)");
    
    // Setup: member on stage, other member in hand
    ...
    // Add second member back to hand to simulate baton touch scenario
    ...
    println!("Q218: Stage has {} members", stage_members);
    
    println!("Q218 verified: Constant ability applies during baton touch");
}
