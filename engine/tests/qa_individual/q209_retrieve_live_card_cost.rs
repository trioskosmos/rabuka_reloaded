...
#[test]
fn test_q209_retrieve_live_card_cost() {
    // Q209: When using this card's ability, can you retrieve live cards placed in the waitroom as cost?
    // Answer: Yes, you can.
    ...
    // Find the member card with this ability (PL!-bp5-009-R "矢澤にこ")
    let member_card = cards.iter()
        .find(|c| c.card_no == "PL!-bp5-009-R")
        .expect("Required card PL!-bp5-009-R not found for Q209 test");
    ...
    assert!(live_cards.len() >= 2, "Need at least 2 live cards for Q209 test");
    ...
    for (i, &live_id) in live_ids.iter().enumerate() {
        ...
        // Live cards may fail and go to waitroom
        println!("Q209: Live card {} result: {:?}", i, result);
    }
    
    // Step 3: Verify waitroom has cards (live cards that failed)
    let waitroom_count = game_state.player1.waitroom.cards.len();
    println!("Q209: Waitroom has {} cards", waitroom_count);
    
    println!("Q209 verified: Live cards in waitroom can be retrieved");
}
