...
#[test]
fn test_q49_turn_order_no_winner() {
    // Q49: What happens to turn order if no one wins live?
    // Answer: Turn order remains the same. If no player places a card in success_live_zone,
    ...
    if let Some(card) = member_card {
        ...
        // The key point: turn order remains the same if no one wins live
        // This test verifies that the turn phase doesn't change when no live winner
        assert_eq!(game_state.current_turn_phase, initial_turn_phase,
            "Turn phase should remain the same when no live winner");
