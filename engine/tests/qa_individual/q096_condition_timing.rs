...
fn test_q096_condition_timing() {
    ...
    if let Some(live) = live_card {
        ...
        if catchu_members.len() >= 2 {
            ...
            if let Some(energy_id) = game_state.player1.energy_zone.pop() {
                game_state.player1.energy_wait.push(energy_id);
            }
            
            // Verify not all energy is active anymore
            assert!(game_state.player1.energy_zone.len() < 6, "Not all energy is active anymore");
        ...
        panic!("Required card PL!SP-pb1-023-L not found for Q096 test");
    }
}
