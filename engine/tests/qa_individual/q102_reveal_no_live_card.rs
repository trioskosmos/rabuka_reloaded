...
fn test_q102_reveal_no_live_card() {
    ...
    if let Some(member) = member_card {
        ...
        let has_live_in_deck = game_state.player1.deck.iter()
        ...
        let has_live_in_discard = game_state.player1.discard_zone.iter()
            .any(|&id| game_state.card_database.get_card(id).map(|c| c.is_live()).unwrap_or(false));
        ...
        let revealed_cards: Vec<_> = game_state.player1.deck.drain(..).collect();
        ...
        let revealed_again: Vec<_> = game_state.player1.deck.drain(..).collect();
        ...
        // Refresh again
        let final_deck: Vec<_> = game_state.player1.discard_zone.drain(..).collect();
        for card_id in final_deck {
            game_state.player1.deck.push(card_id);
        }
        
        // Verify deck has cards after final refresh
        assert!(game_state.player1.deck.len() > 0, "Deck should have cards after final refresh");
        ...
        println!("Initial reveal: {} cards, after refresh reveal: {} cards", revealed_count, revealed_again.len());
    } else {
