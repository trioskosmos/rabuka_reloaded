...
fn test_q122_deck_look_no_refresh() {
    ...
    if let Some(member) = member_card {
        ...
        assert_eq!(game_state.player1.main_deck.cards.len(), 3, "Deck should have 3 cards");
        ...
        for i in 0..cards_to_put_back {
            game_state.player1.main_deck.cards.push(looked_cards[i]);
        }
        
        for i in cards_to_put_back..looked_cards.len() {
            game_state.player1.waitroom.cards.push(looked_cards[i]);
        }
        
        // Verify deck has 2 cards (put back), discard has 1
        assert_eq!(game_state.player1.main_deck.cards.len(), 2, "Deck should have 2 cards");
        assert_eq!(game_state.player1.waitroom.cards.len(), 1, "Discard should have 1 card");
        
        // Deck is not 0, so no refresh
        assert!(game_state.player1.main_deck.cards.len() > 0, "Deck should not be 0");
        ...
        println!("Q122: No member cards found for test");
    }
}
