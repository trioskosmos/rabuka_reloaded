...
fn test_q145_activation_no_target_mandatory() {
    // Q145: Debut ability (cost: wait this member) - add 1 μ's member card from discard to hand
    ...
    // Find the member card with this ability (PL!-bp3-003-R "南ことり")
    ...
    if let Some(member) = member_card {
        ...
        let member_cards_in_discard = game_state.player1.discard_zone.iter()
        ...
        // Try to add μ's member card from discard to hand
        ...
        // Now add a μ's member to discard and try again
        let muse_member = cards.iter()
            .filter(|c| c.is_member())
            .filter(|c| c.group == "μ's")
        ...
        if let Some(muse) = muse_member {
            ...
            game_state.player1.discard_zone.push(muse_id);
            
            // Now there's a μ's member in discard
            let member_cards_in_discard = game_state.player1.discard_zone.iter()
                .filter(|&id| game_state.card_database.get_card(id).map(|c| c.is_member()).unwrap_or(false))
                .filter(|&id| {
            ...
            // Since there's a μ's member in discard, you must add one to hand
            let must_add = member_cards_in_discard > 0;
            
            // Verify you must add one
            assert!(must_add, "Must add μ's member card (one available in discard)");
            ...
            println!("No μ's member in discard: ability resolves, nothing added");
            println!("μ's member in discard: must add one to hand");
        }
    } else {
