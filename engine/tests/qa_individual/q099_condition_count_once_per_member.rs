...
fn test_q099_condition_count_once_per_member() {
    // Q99: Live start automatic ability - for each member on stage who debuted or moved area this turn, reduce need heart by 1
    ...
    let live_card = cards.iter()
        ...
        .next()
        .expect("Required live card not found for Q099 test");
    ...
    let member_card = cards.iter()
        ...
        .next()
        .expect("Required member card not found for Q099 test");
    
    let member_id = get_card_id(member_card, &card_database);
    
    // Setup: Live card in live card zone, member debuted this turn AND moved area
