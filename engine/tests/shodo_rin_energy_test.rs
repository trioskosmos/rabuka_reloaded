...
fn rin_activate_places_energy_under_draws_heart() {
    ...
    assert_eq!(game.state.player1.energy_zone.cards.len(), energy_total_before - 1,
        "1 energy card removed from energy zone");
    assert_eq!(game.state.player1.energy_zone.active_energy_count, active_before - 1,
        "active_energy_count decremented by 1");

    // Card appeared in under_cards of Center area
    let under = game.state.player1.stage.get_under_cards(rabuka_engine::zones::MemberArea::Center);
...
fn rin_activate_across_turns() {
    ...
    game.activate_ability(rin);
    assert_eq!(game.state.player1.stage.get_under_cards(rabuka_engine::zones::MemberArea::Center).len(), 1,
    ...
    for _ in 0..5 { game.pass(); } // P2 Active→Energy→Draw→Main, then LiveCardSet
    ...
    game.activate_ability(rin);
    let final_under = game.state.player1.stage.get_under_cards(rabuka_engine::zones::MemberArea::Center).len();
    // LiveSuccess may move under cards elsewhere; verify at least the new activation placed one.
    assert!(final_under >= 1,
        "at least 1 under after second activation in turn 2 (got {})", final_under);
}
