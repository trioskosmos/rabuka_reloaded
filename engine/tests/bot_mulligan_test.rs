...
pub fn test_mulligan_flow() {
    ...
    if game_state.current_phase == rabuka_engine::game_state::Phase::MulliganP1Turn {
        println!("Player 2 won RPS, auto-chose turn order");
    } else if game_state.current_phase == rabuka_engine::game_state::Phase::ChooseFirstAttacker {
        println!("Player 1 won RPS, choosing turn order");
        let result = turn::TurnEngine::execute_main_phase_action(
            &mut game_state,
            &rabuka_engine::game_setup::ActionType::ChooseFirstAttacker,
            None,
            None,
            None,
            None,
