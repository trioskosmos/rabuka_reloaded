...
fn cheer_pipeline_draw_and_score_icons() {
    ...
    assert!(hand_after >= 1, "Should have cards in hand after live performance");
    ...
    assert!(waitroom_after > waitroom_before, "Yelled cards should be in waitroom after performance (was {}, now {})", waitroom_before, waitroom_after);

    // Verify the cheer_blade_heart_count was set (non-zero since we had blade hearts)
    assert!(game.state.player1_cheer_blade_heart_count > 0,
...
fn cheer_pipeline_score_icon() {
    ...
    assert!(game.state.player1_cheer_blade_heart_count >= 1,
