...
fn get_cost(game: &TestGame, card_id: i16) -> u32 {
    game.state.card_database.get_card(card_id).and_then(|c| c.cost).unwrap_or(99)
}

fn card_name(game: &TestGame, card_id: i16) -> String {
    game.state.card_database.get_card(card_id).map(|c| c.name.clone()).unwrap_or_default()
}
...
fn setup_game() -> (TestGame, i16, Vec<i16>, i16) {
    ...
    (game, yoshiko, vec![cost_2, cost_4, cost_2b, cost_high], filler)
}

/// Test: Cards with cost > 4 should not appear in selection.
...
fn yoshiko_debut_over_cost_filtered() {
    ...
    assert!(game.has_pending_choice(), "Should prompt for optional cost payment");
    game.select_option(1); // pay 4E

    // Should have discard selection (3 matching cards > count=2 → Prompt)
    assert!(game.has_pending_choice(), "Should have discard selection choice");
    
    // Select 2 valid cards
    ...
    // The valid cards should be removed
    assert!(!game.state.player1.waitroom.cards.contains(&cards[0]),
        "card[0] should be removed from discard");
    assert!(!game.state.player1.waitroom.cards.contains(&cards[1]),
        "card[1] should be removed from discard");
}
...
fn yoshiko_debit_single_cost4_ok() {
    ...
    eprintln!("[DEBUG] discard contains cards[0]={}, cards[2]={}, cards[3]={}, discard={:?}",
...
fn yoshiko_debit_stage_full_graceful() {
    ...
    game.state.player1.waitroom.cards.push(cost_2b);
    game.state.player1.waitroom.cards.push(game.id("PL!-sd1-004-SD")); // 3rd valid
    game.state.player1.hand.cards.push(yoshiko);
    game.state.player1.hand.cards.push(filler);
    game.give_energy(16);

    game.play_to_stage(yoshiko, MemberArea::Center);
    if game.has_pending_choice() { game.select_option(1); }
    if game.has_pending_choice() { game.select_indices(&[0, 1]); }

    // Stage full → cards return to discard, NOT hand
    assert!(!game.state.player1.hand.cards.contains(&cost_2a),
        "Should NOT silently route to hand");
    assert!(game.state.player1.waitroom.cards.contains(&cost_2a) ||
            game.state.player1.stage.stage.contains(&cost_2a),
        "Card should be in discard or on stage");
}
