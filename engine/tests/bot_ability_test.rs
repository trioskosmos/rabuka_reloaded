...
pub fn run_ability_test() {
    ...
    let card_database = std::sync::Arc::new(rabuka_engine::card::CardDatabase::load_or_create(cards_vec.clone()));
    ...
    let debut_card_from_all = cards_vec.iter()
        .find(|c| {
            c.card_type == rabuka_engine::card::CardType::Member &&
            c.abilities.iter().any(|a| a.triggers.as_ref().map_or(false, |t| t == rabuka_engine::triggers::DEBUT))
        });
    ...
    let debut_card = game_state.player1.hand.cards.iter().enumerate().find(|(_, card_id)| {
        if let Some(card) = card_database.get_card(**card_id) {
            card.card_type == rabuka_engine::card::CardType::Member &&
            card.abilities.iter().any(|a| a.triggers.as_ref().map_or(false, |t| t == rabuka_engine::triggers::DEBUT))
        } else {
            false
        }
    });
