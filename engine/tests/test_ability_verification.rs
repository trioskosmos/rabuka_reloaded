...
mod ability_verification_tests {
    ...
    fn test_ruby_ability_execution() {
        ...
        // Execute the ability (scoped borrow for executor)
        let result = {
            let mut executor = rabuka_engine::ability::resolver::AbilityResolver::new(&mut game_state);
            executor.resolve_ability(&ability, Some(ruby_id), 0)
        };
        
        match result {
            Ok(_) => {
                println!("✁EAbility executed successfully!");
            ...
            Err(e) => {
                println!("❁EAbility execution failed: {}", e);
            }
        }
    }
    ...
    fn create_test_game_state() -> GameState {
        ...
        let player1 = Player::new("test_player".to_string(), "Test Player".to_string(), false);
        let player2 = Player::new("test_opponent".to_string(), "Test Opponent".to_string(), false);
