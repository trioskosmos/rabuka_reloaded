...
fn kotori_q207_multiname_matches_any_individual_name() {
    ...
    let card = game.db.get_card(multi)
    ...
    let member_count = game.state.player1.stage.stage.iter()
...
fn kotori_q208_multiname_and_single_coexist() {
    ...
    eprintln!("[KOTORI] Multi: {} | Single: {}", card.name,
        game.db.get_card(single_ayumu).map(|c| c.name.as_str()).unwrap_or("?"));
}
