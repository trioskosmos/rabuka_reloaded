...
fn kasumi_debut_skip_optional_no_energy_moved() {
    ...
    assert!(game.has_pending_choice(), "GAP: should prompt optional cost");
    game.select_indices(&[]);
...
fn kasumi_q184_under_energy_not_counted_in_zone() {
    ...
    // Verify under member
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::Center).len(), 2,
        "2 energy under center member");
}
...
fn kasumi_rule_1054_under_energy_recycles_to_energy_deck() {
    ...
    game.state.player1.remove_member_from_stage_with_recycling(1, &game.db);
...
fn kasumi_rule_4553_under_cards_follow_position_change() {
    ...
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::LeftSide).len(), 2);
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::Center).len(), 0);
...
fn mia_activate_retrieves_live_even_without_energy_placement() {
    ...
    game.activate_ability(mia);
    if game.has_pending_choice() { game.select_indices(&[0]); }
    assert!(game.state.player1.hand.cards.contains(&niji_live),
...
fn mia_use_limit_not_enforced() {
    ...
    game.activate_ability(mia);
    if game.has_pending_choice() { game.select_indices(&[0]); }
    assert!(game.state.player1.hand.cards.contains(&niji1), "First activation works");
    ...
    let energy_before = game.state.player1.energy_zone.cards.len() + game.state.player1.stage.get_under_cards(MemberArea::Center).len();
...
fn awakening_move_energy_from_under_to_deck() {
    ...
    game.state.player1.stage.set_area(MemberArea::Center, target);
    place_energy_under(&mut game, MemberArea::Center, 2);
    seed_deck(&mut game);
    let energy_deck_before = game.state.player1.energy_deck.cards.len();
    ...
    assert!(game.has_pending_choice(), "Should have under_member selection choice");
    ...
    let under_after = game.state.player1.stage.get_under_cards(MemberArea::Center).len();

    eprintln!("[AWAKENING] energy deck: {} -> {} (expected +2)", energy_deck_before, energy_deck_after);
    eprintln!("[AWAKENING] under: {} -> {} (expected 0)", under_before, under_after);

    assert_eq!(energy_deck_after, energy_deck_before + 2,
...
fn rina_rule_1053_under_member_goes_to_waitroom() {
    ...
    let target = game.id("PL!N-PR-009-PR");
    game.state.player1.stage.stage[1] = rina;
    game.state.player1.stage.place_under_card(MemberArea::Center, target);
    game.state.player1.remove_member_from_stage_with_recycling(1, &game.db);
...
fn rina_rule_4553_under_member_follows_position_change() {
    ...
    game.state.player1.stage.place_under_card(MemberArea::Center, target);
    game.state.player1.stage.position_change(MemberArea::Center, MemberArea::LeftSide).ok();
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::LeftSide).len(), 1);
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::Center).len(), 0);
...
fn sayaka_activate_effect_does_not_place_under() {
    ...
    let under = game.state.player1.stage.get_under_cards(MemberArea::Center);
    assert_eq!(under.len(), 0, "0 under (reveal→under_member not yet implemented in effect dispatch)");
}
...
fn sayaka_use_limit_enforced() {
    ...
    game.give_energy(3);
    game.activate_ability(sayaka);
    let under_after_first = game.state.player1.stage.get_under_cards(MemberArea::Center).len();
...
fn sayaka_live_start_per_unit_counts_hand_not_under() {
    ...
    for _ in 0..3 {
        ...
        game.state.player1.stage.place_under_card(MemberArea::Center, s);
    }
    seed_deck(&mut game);
    advance_to_live_card_set_p1(&mut game);
    game.set_live_card(filler_live);
...
fn sayaka_rule_4553_under_members_follow_position_change() {
    ...
    let s1 = game.new_id("PL!HS-pb1-002-R");
    let s2 = game.new_id("PL!HS-pb1-002-R");
    game.state.player1.stage.place_under_card(MemberArea::Center, s1);
    game.state.player1.stage.place_under_card(MemberArea::Center, s2);
    game.state.player1.stage.position_change(MemberArea::Center, MemberArea::LeftSide).ok();
    assert_eq!(game.state.player1.stage.get_under_cards(MemberArea::LeftSide).len(), 2);
    assert!(game.state.player1.stage.get_under_cards(MemberArea::LeftSide).contains(&s1));
    assert!(game.state.player1.stage.get_under_cards(MemberArea::LeftSide).contains(&s2));
