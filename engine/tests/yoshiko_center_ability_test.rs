...
fn test_yoshiko_center_ability_basic_success() {
    ...
    let initial_stage_count = game.player().stage.stage.iter().filter(|&&id| id != -1).count();
    ...
    assert!(game.player().stage.stage[1] == yoshiko, "Yoshiko should stay on stage in center");
    assert_eq!(game.player().stage.stage.iter().filter(|&&id| id != -1).count(), initial_stage_count, "Stage count should be same (1 replaced)");
}
...
fn test_yoshiko_center_ability_fails_not_in_center() {
    ...
    assert!(result.is_err(), "Ability activation should fail when not in center");
    
    // Verify no state changes occurred
    assert!(game.player().stage.stage == stage_before, "Stage should be unchanged");
    assert!(game.state.get_pending_choice().is_none(), "Should not have pending choice when not in center");
}
...
fn test_yoshiko_center_ability_fails_no_hand_cards() {
    ...
    assert_eq!(game.player().stage.stage[1], initial_yoshiko_position, "Yoshiko should still be on stage when cost cannot be paid");
}
...
fn test_yoshiko_center_ability_no_other_aqours_on_stage() {
    ...
    assert!(game.player().stage.stage[1] == yoshiko, "Yoshiko should still be on stage but in wait state (cost paid)");
    assert_eq!(game.player().waitroom.cards.len(), initial_discard_size + 1, "Only hand card should be discarded, no stage member moved");
    
    // Verify conditional effect doesn't trigger (no member was moved)
    assert!(game.player().stage.stage[1] == yoshiko, "Yoshiko should remain since main effect failed");
}
...
fn test_yoshiko_center_ability_no_valid_discard_targets() {
    ...
    assert!(game.player().stage.stage[0] == -1, "Chika was moved from stage, area is now empty");
    assert!(game.player().waitroom.cards.contains(&chika), "Chika should be in discard");
...
fn test_yoshiko_center_ability_cost_calculation() {
    ...
    assert!(game.player().waitroom.cards.contains(&riko), "Riko should be in discard");
    
    // Verify main effect worked - Riko was moved from stage
    assert!(game.player().stage.stage[0] == -1, "Riko was moved from stage, area is now empty");
    assert!(game.player().waitroom.cards.contains(&riko), "Riko should be in discard");
...
fn test_yoshiko_center_ability_use_limit() {
    ...
    assert_eq!(game.player().hand.cards.len(), initial_hand_size_after_first, "Should not pay cost again due to use limit");
}
...
fn test_yoshiko_center_ability_exclude_self() {
    ...
    assert!(game.player().waitroom.cards.contains(&chika), "Chika should be the one moved by main effect");
    
    // Verify Yoshiko was NOT moved by main effect (only wait state change)
    assert!(!game.player().waitroom.cards.contains(&yoshiko), "Yoshiko should not be in discard from main effect");
}
