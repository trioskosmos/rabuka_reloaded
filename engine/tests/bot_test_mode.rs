...
fn validate_game_state(game_state: &GameState, card_database: &std::sync::Arc<rabuka_engine::card::CardDatabase>) -> Vec<String> {
    ...
    if p1_success >= 3 && p2_success <= 2 {
        if game_state.game_result != rabuka_engine::game_state::GameResult::FirstAttackerWins {
            issues.push(format!("Game should be over: P1 has {} success cards, P2 has {}", p1_success, p2_success));
        }
    }
    
    if p2_success >= 3 && p1_success <= 2 {
        if game_state.game_result != rabuka_engine::game_state::GameResult::SecondAttackerWins {
...
fn count_stage_members(stage: &rabuka_engine::zones::Stage) -> usize {
    let mut count = 0;
...
pub fn run_test_mode() {
    ...
    if !initial_issues.is_empty() {
        println!("\n⚠�E�E INITIAL VALIDATION ISSUES:");
        for issue in &initial_issues {
            println!("  - {}", issue);
        }
    } else {
        println!("\n✁EInitial state valid");
    }
    ...
    for turn in 1..=max_turns {
        ...
        loop {
            ...
            if phase_loop_count > 100 {
                eprintln!("⚠�E�E Phase loop exceeded 100 iterations, breaking to prevent infinite loop");
            ...
            match current_phase {
                ...
                rabuka_engine::game_state::Phase::LiveStart |
                rabuka_engine::game_state::Phase::LiveSuccess |
                rabuka_engine::game_state::Phase::Cheer => {
                ...
                rabuka_engine::game_state::Phase::ChooseFirstAttacker |
                rabuka_engine::game_state::Phase::MulliganP1Turn |
                rabuka_engine::game_state::Phase::MulliganP1Turn |
                rabuka_engine::game_state::Phase::MulliganP2Turn |
                rabuka_engine::game_state::Phase::LiveCardSetP1Turn |
                rabuka_engine::game_state::Phase::LiveCardSetP2Turn |
        ...
        if actions.is_empty() {
            ...
            for i in 0..std::cmp::min(actions_per_turn, actions.len()) {
                ...
                match turn::TurnEngine::execute_main_phase_action(
                    ...
                    Ok(_) => {
                        println!("✁EAction executed");
                    }
                    Err(e) => {
                        println!("✁EAction failed: {}", e);
                    }
                }
        ...
        // Check for game over
        if game_state.game_result != rabuka_engine::game_state::GameResult::Ongoing {
    ...
    if total_issues == 0 {
        println!("✁ENo issues detected in {} turns", max_turns);
    } else {
        println!("⚠�E�E Found {} issues during gameplay", total_issues);
    }
}
