...
fn hanabiko_stage_to_discard_triggers_ability() {
    ...
    g.state.set_recently_moved_cards(vec![hanabiko]);
...
fn hanabiko_static_discard_no_trigger() {
    ...
    g.state.set_recently_moved_cards(vec![filler]);
...
fn hanabiko_two_copies_one_moved_one_static() {
    ...
    g.state.set_recently_moved_cards(vec![hanabiko_moved]);
    ...
    assert_eq!(
        choice_count, 1,
        "only the moved copy triggers ab#0; static copy must not. got {} choices",
        choice_count
    );
}
