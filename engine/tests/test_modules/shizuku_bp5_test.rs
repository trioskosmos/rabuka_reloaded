...
fn shizuku_bp5_dynamic_energy_payment_tests() {
    ...
    let live_score_2_no = db.cards.values()
    ...
    let live_score_0_no = db.cards.values()
    ...
    // 1. Test case: Normal flow (Live card score = 2, Player has enough energy)
    {
        ...
        assert!(game.has_pending_choice(), "should prompt to select hand card for cost");
        game.select_indices(&[0]); // Discard hand_cost_filler

        // Select live card from waitroom
        assert!(game.has_pending_choice(), "should prompt to select live card from waitroom");
        ...
        // Verify state
        assert_eq!(game.state.player1.energy_zone.active_energy_count, 3, "should pay 2 energy (5 -> 3)");
        assert!(game.state.player1.hand.cards.contains(&discard_live), "live card should be moved to hand");
        assert!(!game.state.player1.waitroom.cards.contains(&discard_live), "live card should no longer be in discard");
    }
    ...
    // 2. Test case: Q214 (Live card score = 0, energy paid = 0)
    {
        ...
        // Verify state
        assert_eq!(game.state.player1.energy_zone.active_energy_count, 1, "should pay 0 energy (1 -> 1)");
        assert!(game.state.player1.hand.cards.contains(&discard_live), "live card should be moved to hand");
        assert!(!game.state.player1.waitroom.cards.contains(&discard_live), "live card should no longer be in discard");
    }
    ...
    // 3. Test case: Insufficient energy (Live card score = 2, Player has 1 energy, choice is skipped or fails)
    {
        ...
        assert!(!game.has_pending_choice(), "should not have pending choice due to insufficient energy");

        // Verify state: Live card is NOT in hand, still in waitroom
        assert_eq!(game.state.player1.energy_zone.active_energy_count, 1, "energy should remain unchanged");
        assert!(!game.state.player1.hand.cards.contains(&discard_live), "live card should NOT be moved to hand");
        assert!(game.state.player1.waitroom.cards.contains(&discard_live), "live card should still be in discard");
    }
    ...
    // 4. Test case: Decline optional payment (Live card score = 2, Player has enough energy but declines)
    {
        ...
        // Verify state: Live card is NOT in hand, still in waitroom
        assert_eq!(game.state.player1.energy_zone.active_energy_count, 5, "energy should remain unchanged");
        assert!(!game.state.player1.hand.cards.contains(&discard_live), "live card should NOT be moved to hand");
        assert!(game.state.player1.waitroom.cards.contains(&discard_live), "live card should still be in discard");
    }
}
