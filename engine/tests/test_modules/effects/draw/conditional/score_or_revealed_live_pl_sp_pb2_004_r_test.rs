...
fn pl_sp_pb2_004_r_live_success_draw_score_or_revealed_live_and_neither() {
    ...
    fire_trigger(&mut game, sumire, AbilityTrigger::LiveSuccess, "ライブ成功時");
    assert_eq!(game.state.player1.hand.cards.len(), 1, "元々のスコアより高いスコアのライブカード → draw 1");
    game.state.mods.add_score_modifier(live6, -2);
    game.state.revealed_cards.push(game.id("PL!-sd1-019-SD"));
    fire_trigger(&mut game, sumire, AbilityTrigger::LiveSuccess, "ライブ成功時");
    assert_eq!(game.state.player1.hand.cards.len(), 2, "revealed scored live → draw 1 via the OR branch");
}
