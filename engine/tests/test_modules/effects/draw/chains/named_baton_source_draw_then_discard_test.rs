...
#[test]
fn n_pb1_019_debut_baton_over_setsuna_draws_two_discards_two() {
    let db = load_real_database();
    run_baton_draw_flow(db, "PL!N-pb1-019-R", "PL!N-PR-009-PR"); // 優木せつ菜
}

#[test]
fn n_pb1_020_debut_baton_over_emma_draws_two_discards_two() {
    let db = load_real_database();
    run_baton_draw_flow(db, "PL!N-pb1-020-R", "PL!N-bp1-020-PRproteinbar"); // エマ・ヴェルデ
}

#[test]
fn n_pb1_019_debut_baton_over_other_name_no_draw() {
    let db = load_real_database();
    let mut game = TestGame::new(db);
