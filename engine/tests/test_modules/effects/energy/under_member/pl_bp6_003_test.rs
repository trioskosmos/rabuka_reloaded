...
fn kotori_live_start_only_muses_under_two_cost_selected() {
    ...
    // The over-cost μ's member must not be selectable: resolve the pick and
    // pin that exactly the cost-2 card goes under (indices are positions
    // within the filtered pool, so position 0 is muse_ok).
    game.select_indices(&[0]);
    assert!(
        game.has_pending_choice(),
        "heart color choice follows the under-placement"
    );
    game.select_option(0);
    assert!(!game.has_pending_choice());
    let under = game
        ...
        .stage
        .get_under_cards(MemberArea::Center);
    assert_eq!(under.len(), 1, "exactly one card goes under");
    assert!(
        under.contains(&muse_ok),
        "only the cost≤2 μ's member goes under"
    );
    assert!(
        game.state.player1.hand.cards.contains(&too_expensive),
        "over-cost member stays in hand"
    );
}
...
fn kotori_live_start_then_live_success_workflow() {
    ...
    // Resolve the LiveStart fully so the workflow's first step pins its
    // outcome (under-placement + heart), not just the prompt shape.
    game.select_indices(&[0]);
    assert!(
        game.has_pending_choice(),
        "heart color choice follows the under-placement"
    ...
    assert!(
        game.state
            ...
            .get_under_cards(MemberArea::Center)
            .contains(&muse),
        "LiveStart places the muse under center"
    );
}
