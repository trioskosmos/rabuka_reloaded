    ...
    while game.has_pending_choice() && picked < 4 {
        let idxs: Vec<usize> = game
            ...
            .filter(|(_, &c)| {
                game.db
                    .get_card(c)
                    .is_some_and(|cc| matches!(cc.card_type, rabuka_engine::card::CardType::Member))
            })
            .map(|(i, _)| i)
            .collect();
        if idxs.is_empty() {
...
fn pl_s_bp7_013_n_choose_opponent_active_player_selects_recycled_members() {
    ...
    assert!(
        game.has_pending_choice(),
        "Card selection from opponent's waitroom expected"
    );
    let entry = game
        .state
        .ability_queue
