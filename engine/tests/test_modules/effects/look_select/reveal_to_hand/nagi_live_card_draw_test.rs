...
fn test_live_card_among_five_drawn_triggers_draw() {
    ...
    assert_eq!(
        discard_size, 5,
        "Should have moved exactly 5 cards to discard"
    );
    assert_eq!(
        final_hand_size,
        initial_hand_size + 1,
        "Should have drawn 1 card because live card was in discard"
    );

    // Verify the live card is actually in discard
    assert!(
        game.player().waitroom.cards.contains(&live_card),
...
fn test_no_live_cards_among_five_drawn_no_draw() {
    ...
    assert_eq!(
        ...
        "Should NOT have drawn any card since no live cards were in discard"
    );
}
...
fn test_live_card_in_existing_discard_but_not_in_five_drawn_no_draw() {
    ...
    assert_eq!(
        final_discard_size,
        initial_discard_size + 5,
        "Should have moved exactly 5 cards to discard"
    );
    assert_eq!(
        final_hand_size, initial_hand_size,
        "Should NOT have drawn any card since none of the 5 drawn cards were live cards"
    );

    // Verify the existing live card is still in discard
    assert!(
        game.player().waitroom.cards.contains(&live_card),
        "Existing live card should still be in discard"
    );
}
...
fn test_multiple_live_cards_among_five_drawn_still_only_draw_one() {
    ...
    assert_eq!(
        final_hand_size,
        initial_hand_size + 1,
        "Should have drawn exactly 1 card even with multiple live cards"
    );

    // Verify both live cards are in discard
    assert!(
        game.player().waitroom.cards.contains(&live_card1),
        "First live card should be in discard"
    );
    assert!(
        game.player().waitroom.cards.contains(&live_card2),
        "Second live card should be in discard"
    );
}
