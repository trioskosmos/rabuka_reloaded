...
fn center_mus_with_2_heart03_reduces_1() {
    ...
    assert_eq!(
        v, -1,
        "honoka 2 heart03 / per_unit_count=2 = 1 unit => -1 heart00, got {v}"
    );
}

#[test]
fn center_mus_zero_heart03_no_reduction() {
    let db = load_real_database();
    let mut game = TestGame::new(db);
    let wonder = game.id("PL!-bp5-020-L");
    ...
    let v = game
        ...
        .mods
        .get_need_heart_modifier(wonder, HeartColor::Heart00);
    assert_eq!(v, 0, "rin 0 heart03 => 0 reduction, got {v}");
}
...
fn no_center_member_no_reduction() {
    ...
    let v = game
        ...
        .mods
        .get_need_heart_modifier(wonder, HeartColor::Heart00);
    assert_eq!(v, 0, "empty stage => condition fail => 0, got {v}");
}
...
fn center_non_mus_no_reduction() {
    ...
    let v = game
        ...
        .mods
        .get_need_heart_modifier(wonder, HeartColor::Heart00);
    assert_eq!(v, 0, "non-μ's center => condition fail => 0, got {v}");
}
