...
fn pl_hs_bp5_013_n_mill_three_all_members_two_blades() {
    ...
    assert_eq!(game.state.mods.get_blade_modifier(me), 2, "all three milled cards are members -> +2 blades");
    assert_eq!(game.state.player1.main_deck.cards.len(), deck_before - 3, "sanity: deck shrank only by the mill");
}
...
fn pl_hs_bp5_013_n_milled_live_card_no_blades() {
    ...
    fire_live_start(&mut game, me);
    assert_eq!(game.state.mods.get_blade_modifier(me), 0, "live card among the milled -> condition fails");
}
