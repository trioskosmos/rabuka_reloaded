...
#[test]
fn nozomi_pl_pb1_007_r_other_lilywhite_member_allows_live_recovery() {
    let db = load_real_database();
    let mut game = TestGame::new(db);
    let me = game.id("PL!-pb1-007-R");
    let mate = game.id("PL!-bp3-014-N");
    ...
    assert!(game.has_pending_choice(), "hand cost prompt expected");
    assert_eq!(
...
#[test]
fn nozomi_pl_pb1_007_r_no_other_lilywhite_member_prevents_live_recovery() {
    let db = load_real_database();
    let mut game = TestGame::new(db);
    let me = game.id("PL!-pb1-007-R");
