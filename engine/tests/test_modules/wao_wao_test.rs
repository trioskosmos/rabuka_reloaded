...
fn wao_wao_q178_activate_3_printemps_score_plus_1() {
    ...
    game.state
        .mods
        .add_orientation_modifier(printemps_b, "wait");
    game.state
        .mods
    ...
    assert_eq!(score_mod, 1, "3 wait members activated → score +1");
}
...
fn wao_wao_q178_already_active_no_change() {
    ...
    let score_mod = game.state.mods.get_score_modifier(wao_wao);
    assert_eq!(score_mod, 0, "0 wait members changed → no score boost");
}
