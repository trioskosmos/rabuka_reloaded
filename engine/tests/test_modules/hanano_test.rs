...
fn hanano_q175_largest_unit_wins_tie_goes_alphabetical() {
    ...
    for _ in 0..20 {
        game.state.player1.main_deck.cards.push(unitless);
    }
    for _ in 0..20 {
        game.state.player2.main_deck.cards.push(unitless);
    }
...
fn hanano_edge_no_unit_has_2_skip() {
    ...
    assert!(!game.has_pending_choice(), "No unit has ≥2 → no choice");
