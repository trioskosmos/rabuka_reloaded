...
fn chisato_cost_hits_threshold_condition_passes() {
    ...
    assert_eq!(
        game.state.revealed_cost_cards.len(),
        5,
    ...
    // use_limit should be consumed (key: (chisato, 0, turn_number))
    let key = (chisato, 0usize, game.state.turn_number);
    assert!(
...
fn chisato_q129_cost_reduction_affects_threshold() {
    ...
    assert!(
        game.state.player1.hand.cards.contains(&ll_card),
