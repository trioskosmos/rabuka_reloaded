...
fn test_yoshiko_debug_move_execution() {
    ...
    println!(
        "For single target case, classify_selection should return Exact with indices: {:?}",
        matching_indices
    );
    ...
    assert!(
        !after_waitroom.contains(&yoshiko),
    ...
    assert!(
        ...
        "Hand card should be in waitroom"
    );
}
