...
fn card_count_condition_negative_wrong_hearts() {
    ...
    let heart_mod = game.state.mods.get_heart_modifier(ginko, HeartColor::Heart04);
    assert_eq!(
        heart_mod,
        0,
...
fn comparison_condition_negative_insufficient_score() {
    ...
    game.state.player2.success_live_card_zone.cards.push(opp_live1);
    game.state.player2.success_live_card_zone.cards.push(opp_live2);
...
fn location_condition_negative_too_few_cards() {
    ...
    game.state.player1.waitroom.cards.push(game.id("PL!N-bp1-001-R")); // Valid target in discard
