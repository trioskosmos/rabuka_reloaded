...
/// Drain auto-ability ordering + card-selection choices until nothing is pending.
/// Mirror the robust loop in auto_system_stress_test (no break; select `count`
/// cards; fall back to index 0).
...
/// Move `target` from its area via a real swap with MOVER, draining autos.
fn area_move_by_swap(game: &mut TestGame, target: i16, target_area: MemberArea) {
    ...
    // Mover occupies a different area so it swaps with target.
