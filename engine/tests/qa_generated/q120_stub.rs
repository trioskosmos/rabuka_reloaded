// Auto-generated test stub for Q120
// Source: qa_data.json
// Question: 『 エールにより公開された自分のカードの中にライブカードが1枚以上あるとき、自分の手札が7枚以下の場合、カードを1枚引く。』について。 自分の手札が7枚の状態で
// Answer: いいえ、この能力の効果でカードを1枚引くことはできません。 発動した自動能力を使うのは、エールで公開された のブレードハートの効果を解決したあとです。 例の場合
// Related cards: "PL!S-bp2-007-R＋", "PL!S-bp2-007-P", "PL!S-bp2-007-P＋", "PL!S-bp2-007-SEC"
...
fn q120_stub() {
    ...
    let card_0 = cards.iter().find(|c: &&Card| c.card_no == "PL!S-bp2-007-R＋").expect("PL!S-bp2-007-R＋");
    let card_1 = cards.iter().find(|c: &&Card| c.card_no == "PL!S-bp2-007-P").expect("PL!S-bp2-007-P");
    let card_2 = cards.iter().find(|c: &&Card| c.card_no == "PL!S-bp2-007-P＋").expect("PL!S-bp2-007-P＋");
    let card_3 = cards.iter().find(|c: &&Card| c.card_no == "PL!S-bp2-007-SEC").expect("PL!S-bp2-007-SEC");
    ...
    println!("Q120 stub executed - manual completion required");
}
