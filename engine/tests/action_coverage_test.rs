...
fn has_action(card: &rabuka_engine::card::Card, target: &str) -> bool {
    for ab in &card.abilities {
        if let Some(ref eff) = ab.effect {
            if eff.action == target { return true; }
            if let Some(ref actions) = eff.compound.actions {
                for sub in actions {
                    if sub.action == target { return true; }
                }
            }
        }
    }
...
fn all_action_types_fire_without_crash() {
    ...
    for (_tid, card) in db.cards.iter() {
        if !matches!(card.card_type, rabuka_engine::card::CardType::Member) { continue; }
        for ab in &card.abilities {
            if let Some(ref eff) = ab.effect {
                if !eff.action.is_empty() { action_types.insert(eff.action.clone()); }
                if let Some(ref actions) = eff.compound.actions {
                    for sub in actions {
                        if !sub.action.is_empty() { action_types.insert(sub.action.clone()); }
                    }
                }
            }
        }
    ...
    for action in &action_list {
        ...
        let card_no = match test_card_no {
            Some(c) => c,
            None => { eprintln!("  [SKIP] {} -- no member card found", action); continue; }
        };
        ...
        let result = TurnEngine::execute_main_phase_action(
            ...
            Some(cid), None, None, None,
        );

        match result {
            Ok(_) => { ok += 1; eprintln!("  [OK] {}", action); }
            Err(e) => {
                if e.contains("No abilities") || e.contains("unknown action") || e.contains("not implemented") {
                    fail_count += 1; eprintln!("  [FAIL] {} -- {}", action, e);
                } else {
                    ok += 1; eprintln!("  [OK] {} (handled: {})", action, e);
                }
            }
        }
    }
    ...
    eprintln!("\n=== Results: {} OK, {} FAIL ===\n", ok, fail_count);
    assert_eq!(fail_count, 0, "{} action types failed to resolve", fail_count);
}
