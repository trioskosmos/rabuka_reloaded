...
fn poppin_q66_has_cards_beats_no_cards() {
    ...
    let p1_score = game.state.player1.live_card_zone.calculate_live_score(
        &game.state.card_database, game.state.player1_cheer_blade_heart_count,
        game.state.player1.stage_hearts.as_ref(), None);
    let p2_score = game.state.player2.live_card_zone.calculate_live_score(
        &game.state.card_database, game.state.player2_cheer_blade_heart_count,
        game.state.player2.stage_hearts.as_ref(), None);
    assert!(p1_score > p2_score,
        "Q66: P1 score ({}) should be > P2 score ({})", p1_score, p2_score);
}
...
fn poppin_lower_score_no_recovery() {
    ...
    game.state.player2.success_live_card_zone.cards.push(other_live);
    ...
    assert!(!game.state.player1.hand.cards.contains(&niji_card),
