...
# Read the CSV and build value-to-row-indices mapping
print("Loading data...")
...
# Pre-build indices: for each column, map value to set of row indices
print("Building indices...")
value_indices = {col: defaultdict(set) for col in columns}
for idx, row in enumerate(rows):
    for col in columns:
        val = row.get(col, '').strip()
        if val:
            value_indices[col][val].add(idx)

print("Indices built. Analyzing...\n")

# Get top values for each column (top 10 by frequency)
column_top_values = {}
for col in columns:
    sorted_values = sorted(value_indices[col].items(), key=lambda x: -len(x[1]))[:10]
    column_top_values[col] = [(val, len(indices)) for val, indices in sorted_values]

# Analyze combinations between different columns (limit to first 10 columns for speed)
print("="*80)
print("TOP 20 COMMON VOTER COMBINATIONS ACROSS DIFFERENT CATEGORIES")
print("="*80 + "\n")

cols_to_analyze = columns[:10]
combinations_data = []
...
for col1, col2 in combinations(cols_to_analyze, 2):
    for val1, count1 in column_top_values[col1]:
        for val2, count2 in column_top_values[col2]:
            # Use set intersection for fast counting
            common = len(value_indices[col1][val1] & value_indices[col2][val2])
            if common > 0:
...
for i, combo in enumerate(combinations_data[:20], 1):
    percentage = (combo['common'] / total_rows) * 100
...
while True:
    ...
    try:
        ...
        # Use pre-built indices for fast analysis
        combo_counts = defaultdict(int)
        for val1, indices1 in value_indices[col1].items():
            for val2, indices2 in value_indices[col2].items():
                common = len(indices1 & indices2)
                if common > 0:
                    combo_counts[(val1, val2)] = common
