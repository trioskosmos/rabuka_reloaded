// Find specific ability #261 to debug the remaining issues
use std::path::Path;

fn main() {
    println!("Finding ability #261...");
            ...
            match serde_json::from_str::<serde_json::Value>(&content) {
                Ok(data) => {
                    if let Some(unique_abilities) = data.get("unique_abilities").and_then(|v| v.as_array()) {
                        if let Some(ability) = unique_abilities.get(261) {
                            println!("Found ability #261:");
