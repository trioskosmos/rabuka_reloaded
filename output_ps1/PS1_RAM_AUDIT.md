...
`build_ps1.bat` runs `tools/bake_deck_cards.py` → `cargo psx build` → copies
`output_ps1/rabuka.ps-exe`. No ISO, no CD-ROM reads, no 532 KB anywhere in RAM.
...
`build_ps1.bat` requires `python` (for `tools/bake_deck_cards.py`) and
