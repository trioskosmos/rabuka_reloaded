    ...
    for idx, a in enumerate(abilities):
        ...
        if '手札を' in t and '控え室に置く' in t:
            ...
            if 'hand' not in sources or 'discard' not in dests:
                fail("WARNING", "hand_to_discard_not_found", idx, t,
                     "source=hand AND dest=discard somewhere in tree",
                     f"sources={sources} dests={dests}")

        # SD2: deck_top source
        if 'デッキの上から' in t or 'デッキの一番上からカードを' in t or 'デッキの一番上のカードを' in t:
            sources = find_in_tree(combined, 'source') or set()
            if 'deck_top' not in sources:
                fail("WARNING", "deck_top_source_missing", idx, t,
        ...
        if ('控え室から' in t or '控え室にある' in t) and '手札に加える' in t:
            ...
            if 'discard' not in sources:
                fail("WARNING", "discard_source_missing", idx, t,
        ...
        if '手札に加える' in t or '手札に置く' in t:
            ...
            if 'hand' not in dests:
                fail("WARNING", "hand_dest_missing", idx, t,
        ...
        if '控え室に置く' in t and '手札を' not in t:
            ...
            if 'discard' not in dests and ('コスト' not in t or 'を' not in t):
                fail("INFO", "discard_dest_missing", idx, t,
        ...
        if 'ステージに置く' in t or '登場させる' in t:
            ...
            if 'stage' not in dests:
                fail("WARNING", "stage_dest_missing", idx, t,
        ...
        if '相手の' in t and '自分の' not in t and '自分と相手' not in t:
            ...
            if 'opponent' not in targets:
                fail("WARNING", "target_opponent_missing", idx, t,
        ...
        if '自分と相手' in t:
            ...
            if 'both' not in targets:
                fail("WARNING", "target_both_missing", idx, t,
                     "target=both in tree",
                     f"targets={targets}")
        ...
        if 'もよい' in t or 'てもよい' in t:
            ...
            if 'True' not in optionals:
                fail("WARNING", "optional_flag_missing", idx, t,
        ...
        if 'すべての' in t or '全ての' in t or '全部の' in t or 'カードをすべて' in t:
            ...
            if 'True' not in alls:
                fail("WARNING", "all_flag_missing", idx, t,
                     "all=True in tree",
                     f"all={alls}")
        ...
        if 'シャッフルする' in t or 'シャッフルして' in t:
            ...
            if 'True' not in shuffles:
                fail("WARNING", "shuffle_flag_missing", idx, t,
                     "shuffle=True in tree",
                     f"shuffle={shuffles}")

        # F4: exclude_self
        if 'このメンバー以外' in t or 'ほかのメンバー' in t:
            excludes = find_in_tree(combined, 'exclude_self') or set()
            if 'True' not in excludes:
                fail("WARNING", "exclude_self_missing", idx, t,
        ...
        if '好きな枚数' in t or '任意の枚数' in t:
            ...
            if 'True' not in any_nums:
                fail("WARNING", "any_number_missing", idx, t,
        ...
        if 'それぞれ' in t or 'ずつ' in t:
            ...
            if 'True' not in mults:
                fail("WARNING", "multiple_targets_missing", idx, t,
        ...
        if '人まで' in t or '枚まで' in t:
            ...
            if 'True' not in maxes:
                fail("WARNING", "max_flag_missing", idx, t,
                     "max=True in tree",
                     f"max={maxes}")
        ...
        if '元々持つ' in t or '元々の' in t:
            ...
            if 'True' not in origs:
                fail("WARNING", "original_value_missing", idx, t,
        ...
        if '好きな順番で' in t:
            ...
            if 'any_order' not in orders:
                fail("WARNING", "placement_order_missing", idx, t,
        ...
        for d in incomplete_moves:
            fail("ERROR", "move_cards_incomplete", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'gain_resource':
                if not d.get('resource'):
                    ...
                    if not d.get('all') and not d.get('per_unit') and not d.get('any_number'):
                        incomplete_gains.append(d)
        walk_tree(combined, cb)
        for d in incomplete_gains:
            fail("ERROR", "gain_resource_incomplete", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'change_state' and not d.get('state_change'):
                fail("ERROR", "change_state_no_state", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'modify_score':
                if not d.get('operation') or not d.get('value'):
                    fail("ERROR", "modify_score_incomplete", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'gain_ability' and not d.get('ability_gain'):
                fail("ERROR", "gain_ability_no_text", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'draw_card' and not d.get('count') and not d.get('dynamic_count'):
                fail("ERROR", "draw_card_no_count", idx, t,
        ...
        def cb(d, _):
            if d.get('action') == 'choice':
                opts = d.get('options', [])
                if len(opts) < 2:
                    fail("WARNING", "choice_too_few_options", idx, t,
        ...
        if '代わりに' in t and '以下から1つを選ぶ' not in t:
            ...
            if 'conditional_alternative' not in acts:
                fail("INFO", "conditional_alternative_not_parsed", idx, t,
        ...
        if 'そうした場合' in t:
            ...
            walk_tree(combined, cb)
            if not found:
                fail("INFO", "conditional_sequential_not_parsed", idx, t,
        ...
        if 'そうしなかった場合' in t:
            ...
            if 'conditional_on_optional' not in acts:
                fail("INFO", "conditional_on_optional_not_parsed", idx, t,
        ...
        if ('たび、' in t or 'たびに' in t) and 'たびに' in t:
            ...
            if 'each_time' not in triggers:
                fail("INFO", "each_time_not_parsed", idx, t,
        ...
        if 'かぎり、' in t:
            ...
            if 'as_long_as' not in durs:
                fail("INFO", "duration_as_long_as_not_parsed", idx, t,
        ...
        if 'につき' in t and 'グループ名につき' not in t:
            if not re.search(r'コスト[はが].*につき.*減る', t):
                ...
                if 'True' not in per_units:
                    fail("INFO", "per_unit_not_parsed", idx, t,
        ...
        if 'これにより' in t and '場合' in t:
            ...
            if 'conditional_on_result' not in acts:
                ...
                if not has_kore:
                    fail("INFO", "kore_niyori_not_parsed", idx, t,
        ...
        if '以下から1つを選ぶ' in t:
            ...
            if 'choice' not in acts:
                fail("WARNING", "choice_not_parsed", idx, t,
        ...
        if '無効に' in t:
            ...
            if 'invalidate_ability' not in acts:
                fail("INFO", "invalidate_not_parsed", idx, t,
        ...
        if 'シャッフルする' in t:
            ...
            if shuffle_without_move:
                fail("INFO", "shuffle_without_move", idx, t,
        ...
        if '{{icon_energy.png|E}}' in t and ('支払う' in t or '支払って' in t):
            ...
            if 'pay_energy' not in acts:
                fail("INFO", "pay_energy_not_parsed", idx, t,
...
def print_report(results):
    ...
    print("=" * 70)
    print(f"SUMMARY: {total} total | {errors} errors | {warnings} warnings | {infos} infos")
