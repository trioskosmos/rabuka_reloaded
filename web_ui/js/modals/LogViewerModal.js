...
export const LogViewerModal = {
    ...
    renderLogs: (container, focusedGroupId) => {
        ...
        // Render groups with enhanced formatting (newest first)
        groupedLogs.reverse();
    ...
    createExpandedLogBlock: (group, currentLang, showFriendlyAbilities, focusedGroupId) => {
        ...
        LogViewerModal._addCardImage(headerDiv, headerEntry);
        ...
        if (group.isChoice) {
            ...
            div.innerHTML = `
                ...
                <div class="log-viewer-content">
                    <div class="log-viewer-choice-head">⚖ ${kindLabel} ${Tooltips.enrichAbilityText(prompt)}${countBadge}</div>
                    ${hasOffered ? `<div class="log-viewer-choice-box">
                        <div class="log-viewer-choice-label">${i18n.t('offered')}</div>
    ...
    enhanceAbilityDescription: (text, currentLang, showFriendlyAbilities, fullEntryRaw = null) => {
        ...
        // Fallback: translate any engine [[...]] markers so raw
        // "[[log_performance:...]]" never reaches the viewer, in either language.
        const plain = (i18n.translateLogMarkers ? i18n.translateLogMarkers(text) : text);
        return {
            condition: '',
            result: plain
        };
...
// Helper: resolve card name from a log entry body
LogViewerModal._resolveCardNameFromBody = (body) => {
    if (!body) return null;
