    ...
    executeDrop: async (source, index, targetZone, targetIndex) => {
        ...
        if (state.pending_choice && state.pending_choice.choice_type === 29) {
            if (source === 'stage' && targetZone === 'stage' && index !== targetIndex) {
                ...
                const p = State.perspectivePlayer === 0 ? state.player1 : state.player2;
                const stage = p.stage;
                const stageArr = [stage.left_side, stage.center, stage.right_side];

                // Local Swap
                const temp = stageArr[index];
                stageArr[index] = stageArr[targetIndex];
                stageArr[targetIndex] = temp;

                // Update stage object
                if (index === 0) stage.left_side = stageArr[0];
                else if (index === 1) stage.center = stageArr[1];
                else if (index === 2) stage.right_side = stageArr[2];

                if (targetIndex === 0) stage.left_side = stageArr[0];
                else if (targetIndex === 1) stage.center = stageArr[1];
                else if (targetIndex === 2) stage.right_side = stageArr[2];
