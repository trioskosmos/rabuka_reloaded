        ...
        if (choice.choice_type === 29) {
            ...
            unique.forEach(item => {
                ...
                if (item.desc) {
                    ...
                    `;
                    cardEl.innerHTML = `
                        <div style="font-weight:bold;color:#cc88ff;margin-bottom:4px;word-break:break-word;overflow-wrap:break-word;">${Tooltips.enrichAbilityText(item.name)}</div>
                        <div style="opacity:0.8;font-size:0.85rem;line-height:1.4;word-break:break-word;overflow-wrap:break-word;">${Tooltips.enrichAbilityText(item.desc)}</div>
                    `;
                } else if (item.isText) {
