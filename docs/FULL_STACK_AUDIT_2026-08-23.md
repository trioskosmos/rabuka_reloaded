...
| 08-25 | (ongoing) | Remaining 2 ignored tests both track bug 13 accept-path: energy zone→deck cost movement silently dropped during cost-resume ({E} processing clobbers pending which-energy selection). Root = R10/R11 cost-funnel territory; decline paths and all other aspects work. Bugs 14/15/17/19 all fixed this session. |
