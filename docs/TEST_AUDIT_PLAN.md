...
7. **Inventory slimming**: DONE — `TEST_INVENTORY.json` (1.7 MB, ~98% of
   generated churn) is excluded from `--check` by default; the four human
   docs still gate normal CI, and `--check --check-json` opts back into
   JSON verification when needed.
