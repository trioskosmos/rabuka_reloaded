...
def test_available_ability():
    """Test the currently available ability action"""
    state, actions = get_state_and_actions()
    if not state:
        return
        
    # Find ability actions
    ability_actions = [a for a in actions if 'use_ability' in a.get('action_type', '')]
    
    if ability_actions:
        print(f"\n=== TESTING AVAILABLE ABILITY ===")
        for i, action in enumerate(ability_actions):
            action_type = action.get('action_type', '')
            description = action.get('description', '')
        ...
        # Test the first ability action
        action = ability_actions[0]
        action_type = action.get('action_type', '')
        action_idx = actions.index(action)
        action_params = action.get('parameters', {})
        
        print(f"\n>>> EXECUTING ABILITY: {action_type}")
        ...
        if success:
            print("ABILITY EXECUTION SUCCESS!")
            examine_current_state()
        else:
            print("ABILITY EXECUTION FAILED")
    else:
        print("No ability actions available")

if __name__ == "__main__":
    examine_current_state()
    test_available_ability()
