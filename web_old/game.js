class MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        const buttonSpacing = Math.max(20, Math.floor(h * 0.025));
        ...
        this.add.text(w / 2, h * 0.35, 'Select Deck:', {
        ...
        const startBtn = this.add.text(w / 2, h * 0.75, 'Start Game', {
...
class GameScene extends Phaser.Scene {
    ...
    createZones() {
        ...
        const zoneGap = Math.max(6, Math.floor(h * 0.008));
        ...
        for (const [key, zone] of Object.entries(this.zones)) {
            ...
            const labelBgHeight = Math.max(18, Math.floor(h * 0.022));
            this.zoneGraphics.fillStyle(0x1a202c, 0.95);
            this.zoneGraphics.fillRoundedRect(zone.x + 5, zone.y + 5, labelBgWidth, labelBgHeight, 5);
            
            const label = this.add.text(zone.x + 12, zone.y + 5 + labelBgHeight / 2, zone.label, {
        ...
        const p1CounterY = p1SupportY - supportHeight - zoneGap - 15;
        const p2CounterY = p2SupportY + supportHeight + zoneGap + 8;
    ...
    createUI() {
        ...
        this.p1HeartIcons = [];
        this.p1HeartCounts = [];
        const iconScale = Math.max(0.25, Math.min(0.4, h * 0.0005));
        ...
        this.p1BladeIcon = this.add.image(45, 0, 'icon_blade').setScale(iconScale).setOrigin(0, 0.5);
        this.p1BladeCount = this.add.text(45 + bladeWidth + 8, 0, '0', {
            fontSize: `${statsSize}px`,
            color: '#48bb78',
            fontStyle: 'bold',
            backgroundColor: '#1a202c',
        ...
        this.p2HeartIcons = [];
        this.p2HeartCounts = [];
        this.p2BladeIcon = this.add.image(65, 0, 'icon_blade').setScale(iconScale).setOrigin(0, 0.5);
        this.p2BladeCount = this.add.text(65 + bladeWidth + 8, 0, '0', {
            fontSize: `${statsSize}px`,
            color: '#718096',
            fontStyle: 'bold',
            backgroundColor: '#1a202c',
        ...
        const phasePadding = Math.max(6, Math.floor(w * 0.008));
        const turnPhaseSpacing = Math.max(60, Math.floor(w * 0.05));
        this.turnText = this.add.text(w - rightPanelWidth - turnPhaseSpacing, headerHeight / 2, 'Turn: 1', {
        ...
        this.phaseText = this.add.text(w - rightPanelWidth - 20, headerHeight / 2, 'Phase: Main', {
    ...
    updateDisplay() {
        ...
        const h = this.scale.height;
        const iconScale = Math.max(0.25, Math.min(0.4, h * 0.0005));
        const heartCountSize = Math.max(11, Math.floor(h * 0.015));
        const heartSpacing = Math.max(25, Math.floor(h * 0.03));
        ...
        for (const [heartType, count] of Object.entries(p1Hearts).sort()) {
            if (count > 0) {
                ...
                const heartCount = this.add.text(xOffset + heartWidth + 8, 0, count.toString(), {
                ...
                xOffset += heartWidth + 8 + Math.max(20, heartCountSize * 2.5); // Spacing based on actual content
            }
        }
        ...
        this.p1BladeCount.setX(xOffset + bladeWidth + 8);
        this.p1BladeCount.setFontSize(`${heartCountSize}px`);
        this.p1BladeCount.setStyle({
            backgroundColor: '#1a202c',
            padding: { x: 4, y: 2 },
            borderRadius: 3
        });
        ...
        this.p2BladeCount.setX(xOffset + bladeWidth + 8);
        this.p2BladeCount.setFontSize(`${heartCountSize}px`);
        this.p2BladeCount.setStyle({
            backgroundColor: '#1a202c',
            padding: { x: 4, y: 2 },
            borderRadius: 3
        });
    ...
    displayHand(zoneKey, cards, interactive) {
        ...
        const padding = Math.max(8, Math.floor(zone.w * 0.015));
    ...
    displayStage(zoneKey, stage, interactive) {
        ...
        const spacing = Math.max(8, Math.floor(zone.w * 0.04));
    ...
    displayZone(zoneKey, cards) {
        ...
        // Calculate dynamic overlap based on available space
        const padding = Math.max(6, Math.floor(zone.w * 0.015));
        const maxTotalWidth = zone.w - padding * 2;
        const cardsTotalWidth = cards.length * cardWidth;
        let overlap;
    ...
    createActionButtons() {
        ...
        const startY = this.actionsPanelY + Math.max(30, Math.floor(h * 0.04));
        ...
        const skipBtnSpacing = Math.max(35, Math.floor(h * 0.045));
        const regularSpacing = Math.max(30, Math.floor(h * 0.04));
        const afterGroupedSpacing = Math.max(18, Math.floor(h * 0.025));
        const afterRegularSpacing = Math.max(10, Math.floor(h * 0.015));
        ...
        this.actions.forEach((action, originalIndex) => {
            ...
            if (params && params.available_areas && params.available_areas.length > 0) {
                ...
                const areaButtonOffset = Math.max(18, Math.floor(h * 0.025));
