...
| `core/game_state/abilities.rs` | `src/core/game_state_abilities.c` | ⚠️ `trigger_auto_abilities` now REAL (scans stage/success/live/hand/energy, fires matching abilities via rb_compound_sequential); record_use/collect_constant_hand/apply_ability_effects/process_pending still TODO |
| `ability/effects/misc.rs` | `src/ability/effects/misc.c` | ⚠️ dispatch + REAL handlers: gain_resource, pay_energy, discard_until_count, rotation, position_change; choice/restriction/re_yell/perform_yell/shuffle/play_baton_touch/place_energy_under_member still no-op |
