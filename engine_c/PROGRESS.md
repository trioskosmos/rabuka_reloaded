...
- [x] **Mass test-port automation** — `tools/gen_tests.py` transpiles the Rust `TestGame` suite (`engine/tests/test_modules`, 3270 `#[test]` fns) into `tests/test_ported_generated.c`. **Current run (this session): 2642 generated fns (was 642). Of those, 1218 emit at least one real CHECK → 548 functions pass / 670 functions fail (checks ok: 779, fail: 670). The other ~1424 generated fns run engine setup but degrade to no resolvable assertion. Hand-ported suite (`tests/test_ported_simple.c`): 13/13 pass.** The generator is a **line-based regex transpiler**, not a Rust→C compiler — see §11 for why it can never be a parity oracle. Remaining ~628 fns are skipped because their body (after inlining per-module helpers) still contains `match` / `if let` / `while let` structural blocks that cannot degrade to a single TODO comment; `place_tang`/TANG and multi-`TestGame` tests are also excluded. To widen coverage this session we: relaxed the skip filter (for/while/loop/`Some`/`=>`/`let(` now degrade line-by-line instead of dropping the fn), added `for`/`while` loop-var declaration, tuple-destructuring var declaration, `None`→0, `game.id(VAR|CONST)` resolution in argument position, `filler_hand(game)`→`test_filler_hand`, `let x = game.has_pending_choice()` and the shared helper getter modifiers (`get_blade/score/cost/heart_modifier(&game,..)`), `assert_eq!(.., Some(N))`, phase `to_string()`/`Phase::X` asserts, `pending_choice_type`, and added C harness surfaces `test_pass`, `test_pending_choice_type`, `test_get_*_modifier`, `test_filler_hand`, `rb_phase_name`, and an `RbPlayer.life` field (life/HP mechanics are still engine-incomplete, so life asserts currently fail). The C engine lacks full life/HP, and closures (`iter().any`/`map_or`/`filter`) + `match`/`if let` remain the hard ceiling.
