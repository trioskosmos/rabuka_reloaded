...
for ab in auto_abilities:
        ...
        if not has_condition and not has_trigger_condition:
            issues.append(
                (
                    ...
                    card_id,
                    tt,
                    "Has したとき/するたび but no condition",
                )
            )
    ...
    if "エールしたとき" in tt:
        ...
        cond_text = json.dumps(cond, ensure_ascii=False) + json.dumps(
            trigger_cond, ensure_ascii=False
        )
        if (
            "revealed" not in cond_text
            and "yell" not in cond_text
            and "custom" not in cond_text
        ):
            issues.append(
                (
                    "YELL_TRIGGER_NOT_CAPTURED",
                    card_id,
                    tt,
    ...
    if "ステージから控え室に置かれたとき" in tt:
        ...
        if loc != "discard":
            issues.append(
                (
                    "DISCARD_LOCATION_WRONG",
                    card_id,
                    tt,
                    f"condition type={cond_type}, location={loc}",
                )
            )
        if cond.get("source") != "preceding_moved" and cond.get("source") != "stage":
            issues.append(
                (
                    "DISCARD_SOURCE_WRONG",
                    card_id,
                    tt,
                    f"Expected source=preceding_moved or source=stage, got {cond.get('source', '?')}",
                )
            )
    ...
    if "登場するたび" in tt:
        trigger_cond = effect.get("trigger_condition", {})
        if (
            effect.get("trigger_type") != "each_time"
            and trigger_cond.get("type") != "appearance_condition"
        ):
            issues.append(
                (
                    "EACH_TIME_APPEAR_WRONG",
                    card_id,
                    tt,
            ...
            if not has_re_yell:
                ...
                if effect.get("action") == "conditional_on_result":
                    ...
                    if followup.get("action") != "re_yell":
                        issues.append(
                            ("RE_YELL_NOT_FOUND", card_id, tt, f"action={action_type}")
                        )
        elif action_type == "conditional_on_result":
            pass  # OK
        else:
            issues.append(
                ("RE_YELL_UNEXPECTED_ACTION", card_id, tt, f"action={action_type}")
            )
    ...
    if "{{turn1.png" in ft or "{{turn2.png" in ft:
        if "{{turn" in tt:
            issues.append(
                (
                    "TURN_LIMIT_NOT_STRIPPED",
                    card_id,
                    tt,
                    "Turn marker still in triggerless",
                )
            )
    ...
    if "対戦相手のカードの効果でも発動する" in tt:
        ...
        if not paren:
            issues.append(
                (
                    "OPPONENT_PARENTHETICAL_MISSING",
                    card_id,
                    tt,
...
if no_trigger_detail:
    ...
    print(
        "  (on_yell, on_move, on_discard, etc.) is ONLY in the text, NOT in the triggers field."
    )
