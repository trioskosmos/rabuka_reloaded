...
def _try_card_count(text):
    for pat, op, unit in [
        ...
        if m:
            ...
            if either_result:
                if 'target' in either_result: result['target'] = either_result['target']
                if 'location' in either_result: result['location'] = either_result['location']
            ...
            if hand_m:
                ...
                if first_text and first_text != text:
                    ...
                    if first_cond and first_cond.get('type') not in ('custom', None):
                        ...
                        for k in ('card_type', 'location', 'target', 'group_names', 'card_property', 'exclude_self'):
                            result.pop(k, None)
...
def _fill_defaults(action, text):
    ...
    if 'position' not in action and 'exclude_position' not in action and 'source_position' not in action:
        for kw, pos in POSITION_KEYWORDS.items():
            if kw in text:
                action['position'] = pos
                break
    if a == 'move_cards' and action.get('destination') == 'hand' and 'source' not in action:
    ...
    if a not in ('move_cards', 'select') and 'or_card_types' not in action:
        card_type_kws = [('live_card', 'ライブカード'), ('member_card', 'メンバーカード'), ('energy_card', 'エネルギーカード')]
        if re.search(r'(ライブカード|メンバーカード|エネルギーカード).*か.*(ライブカード|メンバーカード|エネルギーカード)', text):
            or_types = [t for t, kw in card_type_kws if kw in text]
            if len(or_types) >= 2:
                action['or_card_types'] = or_types
    ...
    if action.get('original_value') and '元々の' in text:
        ...
        op = extract_operator(text)
        if op: action['original_operator'] = op
    if a == 'select' and 'のどちらか' in text:
        or_types = []
        for t, kw in [('live_card', 'ライブカード'), ('member_card', 'メンバーカード'), ('energy_card', 'エネルギーカード')]:
            if kw in text:
                or_types.append(t)
                if t == 'member_card':
                    cl = extract_cost_limit(text)
...
def parse_action(text: str) -> Dict[str, Any]:
    ...
    if is_ability_gain:
        ...
        if quoted_text:
            categorized = categorize_quoted_text(quoted_text)
            if categorized['abilities']:
                action['ability_gain'] = re.sub(r'\{\{[^}]+\}\}', '', categorized['abilities'][0]).replace('「', '').replace('」', '').strip()
    ...
    R(lambda t: '{{icon_energy.png|E}}' in t and ('支払う' in t or '支払って' in t) and '選び' not in t, 'pay_energy', 
    ...
    R(lambda t: re.search(r'コスト[はが](\d+)(減る|減らす|増える|増やす)', t) 
      or re.search(r'ためのコストは(\d+)減る', t), 'modify_cost',
    ...
    R(lambda t: ('置く' in t or '置いて' in t) or ('置き' in t and '置き場' not in t), 'move_cards',
    ...
    R(lambda t: '必要ハート' in t or 'ハートを増やす' in t or 'ハートを減らす' in t, 'modify_required_hearts', None)
...
def _extract_basic_cost_fields(cost, text):
    ...
    if '移動している' in text: cost['movement_state'] = 'has_moved'
    # Baton touch
    if 'バトンタッチ' in text:
        qm = re.search(r'「([^」]+)」からバトンタッチ', text)
        if qm: cost['baton_touch_source'] = qm.group(1)
        gm = re.search(r'『([^』]+)』からバトンタッチ', text)
        if gm: cost['baton_touch_group'] = gm.group(1)
    # Cost limit
    cl = extract_cost_limit(text)
    if cl:
        cost['cost_limit'] = cl
        for kw, op in [('以下', '<='), ('以上', '>='), ('未満', '<'), ('超', '>')]:
            if kw in text: cost['cost_limit_operator'] = op; break
...
def _try_per_unit(text):
    ...
    if pm:
        ...
        if 'ライブ中のカード' in text:
            import sys; print(f"[DEBUG per_unit] OVERRIDE: 枚 → live_card_zone for text={text[:80]}", file=sys.stderr)
    ...
    for kw, loc in [('成功ライブカード置き場にある', 'success_live_zone'), ('ステージにいる', 'stage'), ('控え室にある', 'discard'),
...
def _try_opponent_action(text):
    ...
    opp_action = {'text': oa_text, 'action': 'opponent_action', 'action_by': 'opponent', 'opponent_action': oa}
    if rest:
...
def _try_compound_select(text):
    ...
    actions.append({
        'action': 'select', 'count': 1, 'card_type': 'member_card',
...
def _try_implicit_sequential(text):
    ...
    if '。' in text:
        ...
        clean_for_split = re.sub(r'「[^」]*」', lambda m: m.group(0).replace('。', '\x00'), clean_for_split)
        ...
        filtered = []
        for p in parts:
            if re.match(r'^ライブ終了時まで[、，]?$', p) or re.match(r'^ライブ終了まで[、，]?$', p):
                continue
            filtered.append(p)
        parts = filtered
    else:
    ...
    for p in parts:
        ...
        a = parse_effect(cp)
        if a and a.get('action', 'custom') != 'custom' and a.get('action') != 'do_nothing':
            actions.append(a)
    if len(actions) >= 2:
        return {'text': text, 'action': 'sequential', 'actions': actions}
    return None
...
def _try_choice(text):
    ...
    for line in lines:
        if line.startswith('・'):
            in_opts = True; opts.append(line[1:].strip())
        elif in_opts:
            opts[-1] += ' ' + line
        elif not cond_mod:
            cond_mod = line
    ...
    for ot in opts:
        ...
        if oc and oa: po['condition'] = parse_condition(oc)
        po['text'] = ot
        options.append(po)
    if not options:
        return None
    ...
    if cond_mod and ALTERNATIVE_MARKER in cond_mod:
        ...
        if len(alt_parts) == 2:
            ...
            if '以上' in after:
                ...
                if ac: alt_effect['count'] = ac
            return {
                'text': text, 'action': 'conditional_alternative',
...
def _try_shi_sequential(text):
    ...
    if len(parts) < 2 or not parts[0].strip().endswith('し'):
        return None
    actions = [parse_action(p) for p in parts if parse_action(p).get('action') not in ('custom', 'do_nothing')]
    if len(actions) >= 2:
        return {'text': text, 'action': 'sequential', 'actions': actions}
    return None
...
def _try_restriction_effect(text):
    ...
    if 'アクティブにならない' not in text:
        return None
    result = {'text': text, 'action': 'restriction', 'restriction_type': 'cannot_activate'}
    ...
    if 'エネルギー' in text: result['card_type'] = 'energy_card'
    return result
...
def _try_both_discard_until(text):
    ...
    if '自分と相手はそれぞれ' not in text or '枚になるまで' not in text or ('控え室に置き' not in text and '控え室に置く' not in text):
        return None
        ...
        if result is not None:
            ...
            if result.get('action') == 'choice':
                for opt in result.get('options', []):
                    if dur and 'duration' not in opt and opt.get('action') in ('gain_resource', 'modify_score', 'change_state', 'set_blade_count'):
                        opt['duration'] = dur
                return result
...
def _normalize_effect_tree(effect, original_text=None):
    ...
    def _walk(d, ctx_text=None):
        ...
        if 'activation_position' not in d:
            ...
            for note in parenthetical if isinstance(parenthetical, list) else [parenthetical]:
                if '起動できる' in note or '発動する' in note:
        ...
        if 'exclude_self' not in d and d_ctx and ('このメンバー以外' in d_ctx or 'ほかの' in d_ctx or '他の' in d_ctx):
            d['exclude_self'] = True

        # Propagate distinct from text context — use string form for serde compat
        if 'distinct' not in d and d_ctx and ('名前の異なる' in d_ctx or '異なる名前' in d_ctx):
        ...
        if d.get('action') == 'sequential' and 'actions' in d:
            ...
            for act in acts:
                ...
                if ('すべて' in cond_text or '全部' in cond_text) and prev_count is not None:
                ...
                if 'それらの中に' in cond_text and prev_count is not None:
                    if cond.get('type') == 'card_count_condition' and cond.get('source') is None:
                        cond['source'] = 'preceding_moved'
        ...
        if d.get('position') and d_text:
            d['text'] = re.sub(r'\{\{.+?\.png\|(?:センター|左サイド|右サイド)\}\}', '', d_text).strip()
