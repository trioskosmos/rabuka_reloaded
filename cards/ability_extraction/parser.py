...
def parse_action(text: str) -> Dict[str, Any]:
    ...
    R(
        lambda t: "追加でエールを行う" in t,
        "modify_yell_count",
        lambda t, a: a.update({"operation": "add", "count": 1}),
    )
    R(
...
def _try_character_specific(text):
    ...
    grouped per character into nested sequentials.

    e.g. "「澁谷かのん」1人はheart05+bladeを、「唐可可」1人はheart01+bladeを得る"
    → sequential[ sequential[blade(kanon), heart(kanon)], sequential[blade(keke), heart(keke)] ]
    """
    ...
    # Group per character: each character gets one inner sequential[blade, heart]
    char_groups = []
    for eff in effects:
        ...
        if len(char_actions) == 1:
            char_groups.append(char_actions[0])
        else:
            char_groups.append({"action": "sequential", "actions": char_actions})

    if len(char_groups) == 1:
        result = char_groups[0]
    else:
        result = {"action": "sequential", "actions": char_groups}
    result["text"] = text
...
def _try_or_group_filters(select_text):
    """
    Detect OR-separated group filters with different per-group requirements.
    E.g. "『SunnyPassion』のメンバーカードかブレードハートを持つ『Liella!』のメンバーカード"
    Returns list of option dicts (each with group_names, optional card_type, card_property)
    or None if pattern doesn't match.
    """
    if "か" not in select_text:
        return None
    gns = extract_group_names(select_text)
    if not gns or len(gns) < 2:
        return None
    # Skip if "か" connects card types (already handled by or_card_types)
    card_type_kws = ["ライブカード", "メンバーカード", "エネルギーカード"]
    found_types = [kw for kw in card_type_kws if kw in select_text]
    if len(found_types) >= 2:
        return None
    # Check if the text has a property qualifier (e.g. "ブレードハートを持つ")
    if "ブレードハートを持つ" not in select_text:
        return None
    # Split on first "か" to get the two OR branches
    split_pos = select_text.find("か")
    if split_pos == -1:
        return None
    left_text = select_text[:split_pos].strip()
    right_text = select_text[split_pos + 1 :].strip()
    options = []
    for part_text in [left_text, right_text]:
        part_gns = extract_group_names(part_text)
        if not part_gns:
            continue
        option = {"group_names": part_gns}
        ct = extract_card_type(part_text)
        if ct:
            option["card_type"] = ct
        if "ブレードハートを持つ" in part_text:
            option["card_property"] = "has_blade_heart"
...
def _build_reveal_add_discard(fp, sa_text, select_text):
    ...
    # Check for OR group filters first (e.g. GroupAかpropertyを持つGroupB)
    or_options = _try_or_group_filters(select_text)
    if or_options:
        result["options"] = or_options
        # Don't set group_names — use options instead
    else:
        gns = extract_group_names(select_text)
        if gns:
            result["group_names"] = gns
...
def _enrich_from_text(d, text):
    ...
    if extract_optional(text):
        d["optional"] = True
    # Check for OR group filters first (e.g. GroupAかpropertyを持つGroupB)
    or_options = _try_or_group_filters(text)
    if or_options:
        d["options"] = or_options
        # Don't set group_names — use options instead
    else:
        gns = extract_group_names(text)
        if gns:
...
def _normalize_effect_tree(effect, original_text=None):
    ...
    def _walk(d, ctx_text=None):
        ...
        if "heart_colors" not in d and d.get("action") in (
            ...
            # Don't extract heart_colors when the action already has a
            # specific heart_color assigned (character-specific parsing
            # already set the correct color per character).
            has_heart_color = d.get("action") == "gain_resource" and d.get(
                "heart_color"
            )
            is_non_heart = d.get("action") == "gain_resource" and d.get("resource") in (
                "blade",
                "ブレード",
            )
            if not has_heart_color and not is_non_heart:
        ...
        # Clean up gain_resource actions: remove fields that don't belong
        if d.get("action") == "gain_resource":
            res = d.get("resource")
            if res in ("blade", "ブレード"):
                d.pop("heart_colors", None)
            # source is only meaningful for move_cards, not gain_resource
            d.pop("source", None)
...
def parse_ability(triggerless_text: str) -> Dict[str, Any]:
    ...
    elif "{{left.png|左サイド}}" in triggerless_text:
        extra_pos_from_cost = "left_side"
    elif "{{right.png|右サイド}}" in triggerless_text:
...
def process_abilities(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ability in data["unique_abilities"]:
        ...
        cm = re.search(r"(\d+)以上", cond.get("text", ""))
        if cm and "count" not in cond:
    ...
    # Clean up gain_resource actions: remove inappropriate fields
    ...
    # Fix each_time trigger_conditions: add source:"preceding_moved" when
    # location:"discard" and source is missing (per Q233 - must check moved
    # cards, not static discard count).
    for ability in data["unique_abilities"]:
        eff = ability.get("effect")
        if not isinstance(eff, dict):
            continue
    ...
    # Fix each_time effects: convert sequential[pay_energy(optional), action]
    # to conditional_on_optional (the pay_energy inside sequential doesn't
    # create a proper optional cost prompt).
    for ability in data["unique_abilities"]:
        eff = ability.get("effect")
        if not isinstance(eff, dict):
            continue
        ...
        if not isinstance(first, dict) or not isinstance(second, dict):
            continue
        if first.get("action") == "pay_energy" and first.get("optional") is True:
            eff["action"] = "conditional_on_optional"
            eff["optional_action"] = first
            eff["conditional_action"] = second
            eff.pop("actions", None)

    # Fix existing conditional_on_optional entries that have positive_action
    # instead of conditional_action (migration from old parser output).
    for ability in data["unique_abilities"]:
        eff = ability.get("effect")
        if not isinstance(eff, dict):
            continue
...
def _find_custom_actions(obj, path=""):
    """Recursively find all 'action': 'custom' in a parsed ability tree."""
    found = []
    if isinstance(obj, dict):
        if obj.get("action") == "custom":
            found.append(path)
        for k, v in obj.items():
            found.extend(_find_custom_actions(v, f"{path}.{k}" if path else k))
    elif isinstance(obj, list):
        for i, v in enumerate(obj):
            found.extend(_find_custom_actions(v, f"{path}[{i}]"))
    return found
...
def _validate_semantic(abilities):
    ...
    for i, entry in enumerate(abilities):
        ...
        if not t:
            continue
        # custom actions: parser couldn't determine standard action type
        custom_actions = _find_custom_actions(eff)
        for ca_path in custom_actions:
            issues.append(
                f"  #{i}: custom action at {ca_path} — parser could not determine action type"
            )
