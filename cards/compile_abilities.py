...
# Format: (action_string, [(operand_type, field_name_on_ability_effect)])
# Operand types: "u8", "i8", "u16", "zone", "player", "bool"
EFFECT_DECODE_MAP = {
    "draw_card": (
        "draw_card",
        [
    ...
    "invalidate_ability": ("invalidate_ability", []),
    "suppress_ability_trigger": ("suppress_ability_trigger", []),
    "activate_ability": ("activate_ability", []),
    "play_baton_touch": ("play_baton_touch", []),
...
def generate_vm_rs(build_dir):
    ...
    for json_name, (action, fields) in sorted(EFFECT_DECODE_MAP.items()):
        op = rust_name(json_name)
        lines.append(f"        Opcode::{op} => {{")
        # Read operands (advance cursor)
        reads = []
        for optype, name in fields:
            if name.startswith("_"):
                reads.append(_read_expr(optype, f"_{name[1:]}"))
                continue
            reads.append(_read_expr(optype, name))
        for r in reads:
            lines.append(f"            {r};")
    ...
    # Generate decode_effect_kind for effects that need EffectKind
    # (move_cards with card_type, gain_resource with resource/color, etc.)
    # For now, just return None for all — we'll add them as needed.
    lines.append("/// Decode effect operands into an EffectKind if needed.")
    lines.append("/// Returns None for effects that don't need EffectKind data.")
    lines.append(
