...
fn decode_effect_kind(op: Opcode, cursor: &mut &[u8]) -> Option<Box<EffectKind>> {
    match op {
        Opcode::ActivateAbility => {
            ...
            let count = read_u8_opt(cursor);
            let group_names = read_str_list(cursor);
            let optional = read_bool_opt(cursor);
            let parenthetical = read_str_list(cursor);
            let source_card = read_str(cursor);
            let target = read_str(cursor);
            let target_trigger = read_str(cursor);
            let mut ek = default_abilityOp();
            if let EffectKind::AbilityOp {
                ability_text: ref mut _bc_ability_text,
                group_names: ref mut _bc_group_names,
                source_card: ref mut _bc_source_card,
                target: ref mut _bc_target,
                target_trigger: ref mut _bc_target_trigger,
                ..
            } = &mut ek
            {
                *_bc_ability_text = ability_text.map(|s| s.into());
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(group_names))
                };
        ...
        Opcode::ChangeState => {
            let action_by = read_player_opt(cursor);
            let activation_position = read_zone_opt(cursor);
            let all = read_bool_opt(cursor);
            let blade_limit = read_u8_opt(cursor);
            let blade_limit_operator = read_operator_opt(cursor);
            let card_type = read_card_type_opt(cursor);
            let cost_from_revealed = read_bool_opt(cursor);
            let cost_limit = read_u8_opt(cursor);
            let cost_limit_operator = read_operator_opt(cursor);
            let count = read_u8_opt(cursor);
            let distinct = read_str(cursor);
            let exclude_group_names = read_str_list(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let location = read_zone_opt(cursor);
            let max = read_bool_opt(cursor);
            let optional = read_bool_opt(cursor);
            let original_value = read_bool_opt(cursor);
            let parenthetical = read_str_list(cursor);
            let per_unit = read_bool_opt(cursor);
            let per_unit_count = read_u8_opt(cursor);
            let per_unit_type = read_str(cursor);
            let position = read_zone_opt(cursor);
            let position_compare = read_str(cursor);
            let self_cost = read_bool_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let state = read_heart_opt(cursor);
            let state_change = read_str(cursor);
            let target = read_player_opt(cursor);
            let mut ek = default_changeState();
            if let EffectKind::ChangeState {
                action_by: ref mut _bc_action_by,
                activation_position: ref mut _bc_activation_position,
                all: ref mut _bc_all,
                blade_limit: ref mut _bc_blade_limit,
                blade_limit_operator: ref mut _bc_blade_limit_operator,
                card_type: ref mut _bc_card_type,
                cost_from_revealed: ref mut _bc_cost_from_revealed,
                cost_limit: ref mut _bc_cost_limit,
                cost_limit_operator: ref mut _bc_cost_limit_operator,
                exclude_group_names: ref mut _bc_exclude_group_names,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                location: ref mut _bc_location,
                optional: ref mut _bc_optional,
                original_value: ref mut _bc_original_value,
                per_unit: ref mut _bc_per_unit,
                per_unit_count: ref mut _bc_per_unit_count,
                per_unit_type: ref mut _bc_per_unit_type,
                self_cost: ref mut _bc_self_cost,
                self_target: ref mut _bc_self_target,
                source: ref mut _bc_source,
                target: ref mut _bc_target,
                ..
            } = &mut ek
            {
                *_bc_action_by = action_by.map(|s| s.into());
                *_bc_activation_position = activation_position.map(|s| s.into());
                *_bc_all = all;
                *_bc_blade_limit = blade_limit.map(|v| v as u32);
                *_bc_blade_limit_operator = blade_limit_operator.map(decode_operator_from_str);
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                *_bc_cost_from_revealed = cost_from_revealed;
                *_bc_cost_limit = cost_limit.map(|v| v as u32);
                *_bc_cost_limit_operator = cost_limit_operator.map(decode_operator_from_str);
                *_bc_exclude_group_names = if exclude_group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(exclude_group_names))
                };
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(group_names))
                ...
                *_bc_optional = optional;
                *_bc_original_value = original_value;
                *_bc_per_unit = per_unit;
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
                *_bc_per_unit_type = per_unit_type.map(|s| s.into());
                *_bc_self_cost = self_cost;
                *_bc_self_target = self_target;
                *_bc_source = source.map(|s| s.into());
                *_bc_target = target.map(|s| s.into());
            }
            Some(Box::new(ek))
        }
        Opcode::ChooseTargetPlayer => {
            let choice_options = read_str_list(cursor);
            let optional = read_bool_opt(cursor);
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget {
                choice_options: ref mut _bc_choice_options,
                optional: ref mut _bc_optional,
                ..
            } = &mut ek
            {
                *_bc_choice_options = if choice_options.is_empty() {
                    None
                } else {
                    Some(Box::new(choice_options))
                };
                *_bc_optional = optional;
            }
            Some(Box::new(ek))
        ...
        Opcode::DiscardUntilCount => {
            let baton_touch_trigger = read_bool_opt(cursor);
            let count = read_u8_opt(cursor);
            let destination = read_zone_opt(cursor);
            let multiple_targets = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let target_count = read_u8_opt(cursor);
            let mut ek = default_moveCards();
            if let EffectKind::MoveCards {
                baton_touch_trigger: ref mut _bc_baton_touch_trigger,
                count: ref mut _bc_count,
                destination: ref mut _bc_destination,
                multiple_targets: ref mut _bc_multiple_targets,
                source: ref mut _bc_source,
                target: ref mut _bc_target,
                target_count: ref mut _bc_target_count,
                ..
            } = &mut ek
            {
                *_bc_baton_touch_trigger = baton_touch_trigger;
                *_bc_count = count.map(|v| v as u32);
                *_bc_destination = destination.map(|s| s.into());
                *_bc_multiple_targets = multiple_targets;
                *_bc_source = source.map(|s| s.into());
                *_bc_target = target.map(|s| s.into());
                *_bc_target_count = target_count.map(|v| v as u32);
            }
            Some(Box::new(ek))
        }
        Opcode::DoNothing => {
            let answers = read_str_list(cursor);
            let mut ek = default_customOp();
            if let EffectKind::CustomOp {
                answers: ref mut _bc_answers,
                ..
            } = &mut ek
            {
                *_bc_answers = if answers.is_empty() {
                    None
                } else {
                    Some(Box::new(answers))
                };
            }
            Some(Box::new(ek))
        }
        Opcode::DrawCard => {
            ...
            let count = read_u8_opt(cursor);
            let destination = read_zone_opt(cursor);
            let duration = read_duration_opt(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let heart_colors = read_str_list(cursor);
            let location = read_zone_opt(cursor);
            let multiple_targets = read_bool_opt(cursor);
            let optional = read_bool_opt(cursor);
            let original_value = read_bool_opt(cursor);
            ...
            let per_unit = read_bool_opt(cursor);
            let per_unit_count = read_u8_opt(cursor);
            let per_unit_type = read_zone_opt(cursor);
            let position = read_zone_opt(cursor);
            let position_compare = read_str(cursor);
            let source = read_zone_opt(cursor);
            let state = read_state_opt(cursor);
            let target = read_player_opt(cursor);
            let trigger_type = read_str(cursor);
            let mut ek = default_drawCards();
            if let EffectKind::DrawCards {
                action_by: ref mut _bc_action_by,
                card_type: ref mut _bc_card_type,
                target_count: ref mut _bc_target_count,
                destination: ref mut _bc_destination,
                exclude_self: ref mut _bc_exclude_self,
                heart_colors: ref mut _bc_heart_colors,
                location: ref mut _bc_location,
                original_value: ref mut _bc_original_value,
                per_unit: ref mut _bc_per_unit,
                per_unit_count: ref mut _bc_per_unit_count,
                per_unit_type: ref mut _bc_per_unit_type,
                source: ref mut _bc_source,
                state: ref mut _bc_state,
                target: ref mut _bc_target,
                trigger_type: ref mut _bc_trigger_type,
                ..
            } = &mut ek
            {
                *_bc_action_by = action_by.map(|s| s.into());
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                *_bc_target_count = count.map(|v| v as u32);
                *_bc_destination = destination.map(|s| s.into());
                *_bc_exclude_self = exclude_self;
                *_bc_heart_colors = Box::new(heart_colors);
                *_bc_location = location.map(|s| s.into());
                *_bc_original_value = original_value;
                *_bc_per_unit = per_unit;
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
        ...
        Opcode::GainAbility => {
            ...
            let mut ek = default_abilityOp();
            if let EffectKind::AbilityOp {
                ability_gain: ref mut _bc_ability_gain,
                ability_gain_trigger: ref mut _bc_ability_gain_trigger,
                activation_position: ref mut _bc_activation_position,
                card_type: ref mut _bc_card_type,
                duration: ref mut _bc_duration,
                group_names: ref mut _bc_group_names,
                location: ref mut _bc_location,
                self_target: ref mut _bc_self_target,
                source: ref mut _bc_source,
        ...
        Opcode::GainAbilityFromSource => {
            ...
            let cost_limit_operator = read_operator_opt(cursor);
            let group_names = read_str_list(cursor);
            let source_location = read_zone_opt(cursor);
            let trigger_filter = read_str_list(cursor);
            let mut ek = default_abilityOp();
            if let EffectKind::AbilityOp {
                all: ref mut _bc_all,
                card_type: ref mut _bc_card_type,
                ...
                group_names: ref mut _bc_group_names,
                location: ref mut _bc_location,
                trigger_filter: ref mut _bc_trigger_filter,
                ..
            } = &mut ek
            {
                *_bc_all = all;
                ...
                *_bc_location = source_location.map(|s| s.into());
                *_bc_trigger_filter = if trigger_filter.is_empty() {
                    None
                } else {
                    Some(Box::new(trigger_filter))
                };
            }
            Some(Box::new(ek))
        }
        Opcode::GainResource => {
            ...
            let conditional = read_bool_opt(cursor);
            let cost_limit = read_u8_opt(cursor);
            let cost_limit_operator = read_operator_opt(cursor);
            let count = read_u8_opt(cursor);
            let distinct = read_str(cursor);
            ...
            let exclude_group_names = read_str_list(cursor);
            let exclude_self = read_bool_opt(cursor);
            let filter_targets_by_heart_colors = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let group_reference = read_str(cursor);
            let heart_color = read_heart_opt(cursor);
            let heart_colors = read_str_list(cursor);
            let heart_colors_from_selected_card = read_bool_opt(cursor);
            let heart_type = read_str(cursor);
            let location = read_zone_opt(cursor);
            let max = read_bool_opt(cursor);
            let max_repeats = read_u8_opt(cursor);
            let multiple_targets = read_bool_opt(cursor);
            let negation = read_bool_opt(cursor);
            let original_value = read_bool_opt(cursor);
            let parenthetical = read_str_list(cursor);
            let per_unit = read_bool_opt(cursor);
            let per_unit_count = read_u8_opt(cursor);
            ...
            let per_unit_type = read_zone_opt(cursor);
            let position = read_zone_opt(cursor);
            let require_all_heart_colors = read_bool_opt(cursor);
            let resource = read_str(cursor);
            let same_name = read_bool_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let sign = read_str(cursor);
            let state = read_state_opt(cursor);
            let target = read_player_opt(cursor);
            let target_count = read_u8_opt(cursor);
            let target_from_selection = read_bool_opt(cursor);
            let timing_condition = read_str(cursor);
            let trigger_type = read_str(cursor);
            let mut ek = default_gainResource();
            if let EffectKind::GainResource {
                action_by: ref mut _bc_action_by,
                activation_position: ref mut _bc_activation_position,
                all: ref mut _bc_all,
                card_property: ref mut _bc_card_property,
                card_type: ref mut _bc_card_type,
                characters: ref mut _bc_characters,
                cost_limit: ref mut _bc_cost_limit,
                cost_limit_operator: ref mut _bc_cost_limit_operator,
                value: ref mut _bc_value,
                duration: ref mut _bc_duration,
                exclude_group_names: ref mut _bc_exclude_group_names,
                exclude_self: ref mut _bc_exclude_self,
                filter_targets_by_heart_colors: ref mut _bc_filter_targets_by_heart_colors,
                group_names: ref mut _bc_group_names,
                group_reference: ref mut _bc_group_reference,
                heart_color: ref mut _bc_heart_color,
                heart_colors: ref mut _bc_heart_colors,
                heart_colors_from_selected_card: ref mut _bc_heart_colors_from_selected_card,
                heart_type: ref mut _bc_heart_type,
                location: ref mut _bc_location,
                repeat_limit: ref mut _bc_repeat_limit,
                multiple_targets: ref mut _bc_multiple_targets,
                negation: ref mut _bc_negation,
                original_value: ref mut _bc_original_value,
                per_unit: ref mut _bc_per_unit,
                per_unit_count: ref mut _bc_per_unit_count,
                per_unit_type: ref mut _bc_per_unit_type,
                require_all_heart_colors: ref mut _bc_require_all_heart_colors,
                resource: ref mut _bc_resource,
                same_name: ref mut _bc_same_name,
                self_target: ref mut _bc_self_target,
                sign: ref mut _bc_sign,
                state: ref mut _bc_state,
                target_count: ref mut _bc_target_count,
                target_from_selection: ref mut _bc_target_from_selection,
                timing_condition: ref mut _bc_timing_condition,
                trigger_type: ref mut _bc_trigger_type,
                ..
            } = &mut ek
            {
                ...
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                *_bc_characters = if characters.is_empty() {
                    None
                } else {
                    Some(Box::new(characters))
                };
                *_bc_cost_limit = cost_limit.map(|v| v as u32);
                *_bc_cost_limit_operator = cost_limit_operator.map(decode_operator_from_str);
                *_bc_value = count.map(|v| v as u32);
                *_bc_duration = duration.map(|s| s.into());
                *_bc_exclude_group_names = if exclude_group_names.is_empty() {
                    None
                } else {
                ...
                *_bc_exclude_self = exclude_self;
                *_bc_filter_targets_by_heart_colors = filter_targets_by_heart_colors;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(group_names))
                ...
                *_bc_heart_color = heart_color.map(|s| s.into());
                *_bc_heart_colors = Box::new(heart_colors);
                *_bc_heart_colors_from_selected_card = heart_colors_from_selected_card;
                *_bc_heart_type = heart_type.map(|s| s.into());
                *_bc_location = location.map(|s| s.into());
                *_bc_repeat_limit = max_repeats.map(|v| v as u32);
                *_bc_multiple_targets = multiple_targets;
                *_bc_negation = negation;
                *_bc_original_value = original_value;
                *_bc_per_unit = per_unit;
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
                *_bc_per_unit_type = per_unit_type.map(|s| s.into());
                *_bc_require_all_heart_colors = require_all_heart_colors;
                *_bc_resource = resource.map(|s| s.into());
                *_bc_same_name = same_name;
                *_bc_self_target = self_target;
                *_bc_sign = sign.map(|s| s.into());
                *_bc_state = state;
                *_bc_target_count = target_count.map(|v| v as u32);
                *_bc_target_from_selection = target_from_selection;
        ...
        Opcode::ModifyCost => {
            ...
            let group_names = read_str_list(cursor);
            let location = read_zone_opt(cursor);
            let non_stackable = read_bool_opt(cursor);
            let operation = read_str(cursor);
            let original_count = read_u8_opt(cursor);
            let original_operator = read_operator_opt(cursor);
            let original_value = read_bool_opt(cursor);
            let per_unit = read_bool_opt(cursor);
            let per_unit_count = read_u8_opt(cursor);
            let per_unit_location = read_zone_opt(cursor);
            let per_unit_type = read_zone_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let value = read_u8_opt(cursor);
            let mut ek = default_customOp();
            if let EffectKind::CustomOp {
                card_type: ref mut _bc_card_type,
                exclude_self: ref mut _bc_exclude_self,
        ...
        Opcode::ModifyRequiredHearts => {
            ...
            let distinct = read_str(cursor);
            let duration = read_str(cursor);
            let exclude_heart_colors = read_str_list(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let heart_colors = read_str_list(cursor);
            let location = read_zone_opt(cursor);
            ...
            let per_unit = read_bool_opt(cursor);
            let per_unit_count = read_u8_opt(cursor);
            let per_unit_heart_colors = read_str_list(cursor);
            let per_unit_type = read_str(cursor);
            let position = read_zone_opt(cursor);
            let replace_all = read_bool_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let target = read_player_opt(cursor);
            let timing_condition = read_str(cursor);
            let value = read_u8_opt(cursor);
            let mut ek = default_modifyHearts();
            if let EffectKind::ModifyHearts {
                card_type: ref mut _bc_card_type,
                value: ref mut _bc_value,
                duration: ref mut _bc_duration,
                exclude_heart_colors: ref mut _bc_exclude_heart_colors,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                heart_colors: ref mut _bc_heart_colors,
                location: ref mut _bc_location,
                repeat_limit: ref mut _bc_repeat_limit,
                operation: ref mut _bc_operation,
                original_count: ref mut _bc_original_count,
                original_operator: ref mut _bc_original_operator,
                original_value: ref mut _bc_original_value,
                per_unit: ref mut _bc_per_unit,
                per_unit_count: ref mut _bc_per_unit_count,
                per_unit_heart_colors: ref mut _bc_per_unit_heart_colors,
                per_unit_type: ref mut _bc_per_unit_type,
                replace_all: ref mut _bc_replace_all,
                self_target: ref mut _bc_self_target,
                timing_condition: ref mut _bc_timing_condition,
                ..
            } = &mut ek
            {
                ...
                *_bc_value = count.map(|v| v as u32);
                *_bc_duration = duration.map(|s| s.into());
                *_bc_exclude_heart_colors = if exclude_heart_colors.is_empty() {
                    None
                } else {
                    Some(Box::new(exclude_heart_colors))
                };
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                ...
                *_bc_original_count = original_count.map(|v| v as u32);
                *_bc_original_operator = original_operator.map(decode_operator_from_str);
                *_bc_original_value = original_value;
                *_bc_per_unit = per_unit;
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
                *_bc_per_unit_heart_colors = Box::new(per_unit_heart_colors);
                *_bc_per_unit_type = per_unit_type.map(|s| s.into());
                *_bc_replace_all = replace_all;
                *_bc_self_target = self_target;
                *_bc_timing_condition = timing_condition.map(|s| s.into());
            }
            Some(Box::new(ek))
        ...
        Opcode::ModifyRequiredHeartsGlobal => {
            ...
            } = &mut ek
            {
                ...
                *_bc_heart_colors = Box::new(heart_colors);
                *_bc_operation = operation.map(|s| s.into());
                *_bc_value = value.map(|v| v as u32);
            }
            Some(Box::new(ek))
        }
        Opcode::ModifyScore => {
            let activation_position = read_zone_opt(cursor);
            let card_names = read_str_list(cursor);
            let card_property = read_str(cursor);
            let card_type = read_card_type_opt(cursor);
            let conditional = read_bool_opt(cursor);
            let distinct = read_str(cursor);
            ...
            let max_repeats = read_u8_opt(cursor);
            let multiple_targets = read_bool_opt(cursor);
            let need_heart_operator = read_operator_opt(cursor);
            let need_heart_total = read_u8_opt(cursor);
            let negation = read_bool_opt(cursor);
            let operation = read_str(cursor);
            let parenthetical = read_str_list(cursor);
            let per_unit = read_bool_opt(cursor);
            ...
            if let EffectKind::ModifyScore {
                activation_position: ref mut _bc_activation_position,
                card_names: ref mut _bc_card_names,
                card_property: ref mut _bc_card_property,
                card_type: ref mut _bc_card_type,
                duration: ref mut _bc_duration,
                group_names: ref mut _bc_group_names,
                heart_colors: ref mut _bc_heart_colors,
                location: ref mut _bc_location,
                max_repeats: ref mut _bc_max_repeats,
                need_heart_operator: ref mut _bc_need_heart_operator,
                need_heart_total: ref mut _bc_need_heart_total,
                negation: ref mut _bc_negation,
                operation: ref mut _bc_operation,
                per_unit: ref mut _bc_per_unit,
                per_unit_count: ref mut _bc_per_unit_count,
            ...
            {
                *_bc_activation_position = activation_position.map(|s| s.into());
                *_bc_card_names = Box::new(card_names);
                *_bc_card_property = card_property.map(|s| s.into());
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                *_bc_duration = duration.map(|s| s.into());
                *_bc_group_names = if group_names.is_empty() {
                ...
                *_bc_heart_colors = Box::new(heart_colors);
                *_bc_location = location.map(|s| s.into());
                *_bc_max_repeats = max_repeats.map(|v| v as u32);
                *_bc_need_heart_operator = need_heart_operator.map(decode_operator_from_str);
                *_bc_need_heart_total = need_heart_total.map(|v| v as u32);
                *_bc_negation = negation;
                *_bc_operation = operation.map(|s| s.into());
                *_bc_per_unit = per_unit;
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
        ...
        Opcode::MoveCards => {
            ...
            let activation_position = read_zone_opt(cursor);
            let all = read_bool_opt(cursor);
            let allow_occupied_stage = read_bool_opt(cursor);
            let answers = read_str_list(cursor);
            let any_number = read_bool_opt(cursor);
            let baton_touch_trigger = read_bool_opt(cursor);
            let card_names = read_str_list(cursor);
            let card_property = read_str(cursor);
            let card_type = read_card_type_opt(cursor);
            let characters = read_str_list(cursor);
            let cost_limit = read_u8_opt(cursor);
            let cost_limit_max = read_u8_opt(cursor);
            let cost_limit_min = read_u8_opt(cursor);
            let cost_limit_operator = read_operator_opt(cursor);
            let cost_offset = read_u8_opt(cursor);
            let cost_reference = read_str(cursor);
            let cost_total = read_u8_opt(cursor);
            let cost_total_operator = read_operator_opt(cursor);
            let count = read_u8_opt(cursor);
            let destination = read_zone_opt(cursor);
            let discard_remaining = read_bool_opt(cursor);
            let distinct = read_str(cursor);
            let duration = read_str(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            ...
            let need_heart_color = read_heart_opt(cursor);
            let need_heart_operator = read_operator_opt(cursor);
            let need_heart_total = read_u8_opt(cursor);
            let negation = read_bool_opt(cursor);
            let optional = read_bool_opt(cursor);
            let or_card_types = read_str_list(cursor);
            let parenthetical = read_str_list(cursor);
            let placement_order = read_str(cursor);
            let position = read_zone_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let shuffle = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let state_change = read_state_opt(cursor);
            let target = read_zone_opt(cursor);
            let mut ek = default_moveCards();
            if let EffectKind::MoveCards {
                action_by: ref mut _bc_action_by,
                activation_position: ref mut _bc_activation_position,
                all: ref mut _bc_all,
                allow_occupied_stage: ref mut _bc_allow_occupied_stage,
                any_number: ref mut _bc_any_number,
                baton_touch_trigger: ref mut _bc_baton_touch_trigger,
                card_names: ref mut _bc_card_names,
                card_property: ref mut _bc_card_property,
                card_type: ref mut _bc_card_type,
                characters: ref mut _bc_characters,
                cost_limit: ref mut _bc_cost_limit,
                cost_limit_max: ref mut _bc_cost_limit_max,
                cost_limit_min: ref mut _bc_cost_limit_min,
                cost_limit_operator: ref mut _bc_cost_limit_operator,
                cost_offset: ref mut _bc_cost_offset,
                cost_reference: ref mut _bc_cost_reference,
                cost_total: ref mut _bc_cost_total,
                cost_total_operator: ref mut _bc_cost_total_operator,
                count: ref mut _bc_count,
                destination: ref mut _bc_destination,
                discard_remaining: ref mut _bc_discard_remaining,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                group_reference: ref mut _bc_group_reference,
                heart_colors: ref mut _bc_heart_colors,
                location: ref mut _bc_location,
                multiple_targets: ref mut _bc_multiple_targets,
                name_constraint: ref mut _bc_name_constraint,
                name_constraint_source: ref mut _bc_name_constraint_source,
                need_heart_color: ref mut _bc_need_heart_color,
                need_heart_operator: ref mut _bc_need_heart_operator,
                need_heart_total: ref mut _bc_need_heart_total,
                negation: ref mut _bc_negation,
                or_card_types: ref mut _bc_or_card_types,
                self_target: ref mut _bc_self_target,
                shuffle: ref mut _bc_shuffle,
                source: ref mut _bc_source,
            ...
            } = &mut ek
            {
                ...
                *_bc_activation_position = activation_position.map(|s| s.into());
                *_bc_all = all;
                *_bc_allow_occupied_stage = allow_occupied_stage;
                *_bc_any_number = any_number;
                *_bc_baton_touch_trigger = baton_touch_trigger;
                *_bc_card_names = Box::new(card_names);
                *_bc_card_property = card_property.map(|s| s.into());
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                ...
                *_bc_cost_limit = cost_limit.map(|v| v as u32);
                *_bc_cost_limit_max = cost_limit_max.map(|v| v as u32);
                *_bc_cost_limit_min = cost_limit_min.map(|v| v as u32);
                *_bc_cost_limit_operator = cost_limit_operator.map(decode_operator_from_str);
                *_bc_cost_offset = cost_offset.map(|v| v as i32);
                *_bc_cost_reference = cost_reference.map(|s| s.into());
                *_bc_cost_total = cost_total.map(|v| v as u32);
                *_bc_cost_total_operator = cost_total_operator.map(decode_operator_from_str);
                *_bc_count = count.map(|v| v as u32);
                *_bc_destination = destination.map(|s| s.into());
                *_bc_discard_remaining = discard_remaining;
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                ...
                *_bc_need_heart_color = need_heart_color.map(|s| s.into());
                *_bc_need_heart_operator = need_heart_operator.map(decode_operator_from_str);
                *_bc_need_heart_total = need_heart_total.map(|v| v as u32);
                *_bc_negation = negation;
                *_bc_or_card_types = if or_card_types.is_empty() {
                    None
                } else {
                    Some(Box::new(or_card_types))
                };
                *_bc_self_target = self_target;
                *_bc_shuffle = shuffle;
                *_bc_source = source.map(|s| s.into());
                *_bc_state_change = state_change;
                *_bc_target = target.map(|s| s.into());
            }
            Some(Box::new(ek))
        }
        ...
        Opcode::PlaceEnergyUnderMember => {
            ...
            let count = read_u8_opt(cursor);
            let destination = read_zone_opt(cursor);
            let energy_count = read_u8_opt(cursor);
            let group_names = read_str_list(cursor);
            let optional = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let state_change = read_state_opt(cursor);
            ...
            if let EffectKind::MoveCards {
                ...
                count: ref mut _bc_count,
                destination: ref mut _bc_destination,
                energy_count: ref mut _bc_energy_count,
                group_names: ref mut _bc_group_names,
                source: ref mut _bc_source,
                state_change: ref mut _bc_state_change,
                target: ref mut _bc_target,
                target_member: ref mut _bc_target_member,
                ..
            } = &mut ek
            {
                *_bc_any_number = any_number;
                ...
                *_bc_count = count.map(|v| v as u32);
                *_bc_destination = destination.map(|s| s.into());
                *_bc_energy_count = energy_count.map(|v| v as u32);
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(group_names))
        ...
        Opcode::PositionChange => {
            ...
            let count = read_u8_opt(cursor);
            let destination = read_zone_opt(cursor);
            let exclude_position = read_zone_opt(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let multiple_targets = read_bool_opt(cursor);
            let optional = read_bool_opt(cursor);
            ...
            let position_compare = read_str(cursor);
            let source = read_zone_opt(cursor);
            let source_position = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let target_member = read_str(cursor);
            let mut ek = default_positionOp();
            if let EffectKind::PositionOp {
                card_type: ref mut _bc_card_type,
                destination: ref mut _bc_destination,
                exclude_position: ref mut _bc_exclude_position,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                multiple_targets: ref mut _bc_multiple_targets,
                optional: ref mut _bc_optional,
                source: ref mut _bc_source,
                source_position: ref mut _bc_source_position,
                target: ref mut _bc_target,
                target_member: ref mut _bc_target_member,
                ..
            } = &mut ek
            {
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                *_bc_destination = destination.map(|s| s.into());
                *_bc_exclude_position = exclude_position.map(|s| s.into());
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                ...
                *_bc_optional = optional;
                *_bc_source = source.map(|s| s.into());
                *_bc_source_position = source_position.map(|s| s.into());
                *_bc_target = target.map(|s| s.into());
                *_bc_target_member = target_member.map(|s| s.into());
            }
            Some(Box::new(ek))
        ...
        Opcode::ReYell => {
            let lose_blade_hearts = read_bool_opt(cursor);
            let target = read_player_opt(cursor);
            let mut ek = default_miscOp();
            if let EffectKind::MiscOp {
                lose_blade_hearts: ref mut _bc_lose_blade_hearts,
                target: ref mut _bc_target,
                ..
            } = &mut ek
            {
                *_bc_lose_blade_hearts = lose_blade_hearts;
                *_bc_target = target.map(|s| s.into());
            }
            Some(Box::new(ek))
        }
        Opcode::ReduceLiveCardSetLimit => {
            ...
            if let EffectKind::RestrictionOp {
                card_type: ref mut _bc_card_type,
                ..
            } = &mut ek
            {
        ...
        Opcode::Restriction => {
            let card_type = read_card_type_opt(cursor);
            let count = read_u8_opt(cursor);
            let delayed = read_bool_opt(cursor);
            let destination = read_zone_opt(cursor);
            let duration = read_duration_opt(cursor);
            let exclude_group_names = read_str_list(cursor);
            ...
            if let EffectKind::RestrictionOp {
                card_type: ref mut _bc_card_type,
                delayed: ref mut _bc_delayed,
                duration: ref mut _bc_duration,
                exclude_group_names: ref mut _bc_exclude_group_names,
                phase: ref mut _bc_phase,
                restriction_type: ref mut _bc_restriction_type,
                self_target: ref mut _bc_self_target,
                ..
            } = &mut ek
            {
        ...
        Opcode::Reveal => {
            let activation_position = read_zone_opt(cursor);
            let all = read_bool_opt(cursor);
            let blind = read_bool_opt(cursor);
            let card_type = read_card_type_opt(cursor);
            let cost_limit = read_u8_opt(cursor);
            let cost_limit_operator = read_operator_opt(cursor);
            let count = read_u8_opt(cursor);
            ...
            let per_unit_count = read_u8_opt(cursor);
            let per_unit_type = read_str(cursor);
            let picker = read_player_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let mut ek = default_lookReveal();
            ...
            } = &mut ek
            {
                ...
                *_bc_per_unit_count = per_unit_count.map(|v| v as u32);
                *_bc_per_unit_type = per_unit_type.map(|s| s.into());
                *_bc_picker = picker.map(|s| s.into());
                *_bc_self_target = self_target;
                *_bc_source = source.map(|s| s.into());
                *_bc_target = target.map(|s| s.into());
            }
        ...
        Opcode::Select => {
            let ability_filter = read_str(cursor);
            let ability_filter_triggers = read_str_list(cursor);
            let action_by = read_player_opt(cursor);
            let activation_position = read_zone_opt(cursor);
            let all = read_bool_opt(cursor);
            let card_type = read_card_type_opt(cursor);
            ...
            let distinct = read_str(cursor);
            let duration = read_duration_opt(cursor);
            let exclude_selected = read_bool_opt(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let heart_colors = read_str_list(cursor);
            let optional = read_bool_opt(cursor);
            ...
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget {
                ability_filter_triggers: ref mut _bc_ability_filter_triggers,
                action_by: ref mut _bc_action_by,
                activation_position: ref mut _bc_activation_position,
                card_type: ref mut _bc_card_type,
                characters: ref mut _bc_characters,
                ...
                cost_limit_operator: ref mut _bc_cost_limit_operator,
                target_count: ref mut _bc_target_count,
                exclude_selected: ref mut _bc_exclude_selected,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                heart_colors: ref mut _bc_heart_colors,
                optional: ref mut _bc_optional,
            ...
            {
                *_bc_ability_filter_triggers = if ability_filter_triggers.is_empty() {
                    None
                } else {
                    Some(ability_filter_triggers)
                };
                *_bc_action_by = action_by.map(|s| s.into());
                *_bc_activation_position = activation_position.map(|s| s.into());
                *_bc_card_type = card_type.map(EffectCardType::from_str);
                ...
                *_bc_cost_limit_operator = cost_limit_operator.map(decode_operator_from_str);
                *_bc_target_count = count.map(|v| v as u32);
                *_bc_exclude_selected = exclude_selected;
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
        ...
        Opcode::SelectCards => {
            ...
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let heart_color_count = read_u8_opt(cursor);
            let heart_colors = read_str_list(cursor);
            let max = read_bool_opt(cursor);
            let optional = read_bool_opt(cursor);
            let or_card_types = read_str_list(cursor);
            let original_value = read_bool_opt(cursor);
            let per_group = read_bool_opt(cursor);
            let per_group_count = read_u8_opt(cursor);
            let placement_order = read_str(cursor);
            let remainder_destination = read_zone_opt(cursor);
            let require_all_heart_colors = read_bool_opt(cursor);
            let reveal = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget {
                any_number: ref mut _bc_any_number,
                ...
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                heart_color_count: ref mut _bc_heart_color_count,
                heart_colors: ref mut _bc_heart_colors,
                optional: ref mut _bc_optional,
                or_card_types: ref mut _bc_or_card_types,
                original_value: ref mut _bc_original_value,
                per_group: ref mut _bc_per_group,
                per_group_count: ref mut _bc_per_group_count,
                require_all_heart_colors: ref mut _bc_require_all_heart_colors,
                reveal: ref mut _bc_reveal,
                source: ref mut _bc_source,
                ..
            } = &mut ek
            {
                ...
                *_bc_heart_color_count = heart_color_count.map(|v| v as u32);
                *_bc_heart_colors = Box::new(heart_colors);
                *_bc_optional = optional;
                *_bc_or_card_types = if or_card_types.is_empty() {
                    None
                ...
                *_bc_original_value = original_value;
                *_bc_per_group = per_group;
                *_bc_per_group_count = per_group_count.map(|v| v as u32);
                *_bc_require_all_heart_colors = require_all_heart_colors;
                *_bc_reveal = reveal;
                *_bc_source = source.map(|s| s.into());
            }
        ...
        Opcode::SetCardIdentity => {
            let all = read_bool_opt(cursor);
            let all_regions = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let identities = read_str_list(cursor);
            let self_target = read_bool_opt(cursor);
            let mut ek = default_changeState();
            if let EffectKind::ChangeState {
                all: ref mut _bc_all,
                all_regions: ref mut _bc_all_regions,
                group_names: ref mut _bc_group_names,
                identities: ref mut _bc_identities,
                self_target: ref mut _bc_self_target,
                ..
            } = &mut ek
            {
                *_bc_all = all;
                *_bc_all_regions = all_regions;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                    Some(Box::new(group_names))
                };
                *_bc_identities = if identities.is_empty() {
                    None
                } else {
                    Some(Box::new(identities))
                };
                *_bc_self_target = self_target;
            }
            Some(Box::new(ek))
        ...
        Opcode::SpecifyHeartColor => {
            let activation_position = read_zone_opt(cursor);
            let choice = read_bool_opt(cursor);
            let count = read_u8_opt(cursor);
            let position = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let mut ek = default_miscOp();
        ...
        Opcode::SuppressAbilityTrigger => {
            ...
            if let EffectKind::AbilityOp {
                card_type: ref mut _bc_card_type,
                source: ref mut _bc_source,
                suppressed_trigger: ref mut _bc_suppressed_trigger,
                target: ref mut _bc_target,
                ..
            } = &mut ek
            {
...
fn action_for_op(op: Opcode) -> &'static str {
    match op {
        ...
        Opcode::CompoundChoice => "compound_choice",
        Opcode::CompoundConditional => "compound_conditional",
        Opcode::CompoundConditionalAlt => "compound_conditional_alt",
        Opcode::CompoundLookAt => "compound_look_at",
        Opcode::CompoundSelectCards => "compound_select_cards",
        Opcode::CompoundSequential => "compound_sequential",
...
pub fn decode_condition(cursor: &mut &[u8]) -> Condition {
    ...
    match op_val {
        81 => {
            ...
            if let Condition::AbilityFilter {
                text: ref mut _bc_text,
                ..
            } = &mut c
            {
        ...
        80 => {
            let _ = read_u8(cursor);
            let operator = read_operator_opt(cursor);
            let count = read_u16_opt(cursor);
            let mut c = default_condition_comparison();
            if let Condition::Comparison {
                operator: ref mut _bc_operator,
                count: ref mut _bc_count,
                ..
            } = &mut c
            {
                *_bc_operator = operator.map(|s| s.into());
                *_bc_count = count.map(|v| v as u32);
            }
            c
        }
        ...
        64 => {
            ...
            let operator = read_operator_opt(cursor);
            let count = read_u8_opt(cursor);
            let card_type = read_u8_opt(cursor);
            ...
            } = &mut c
            {
                ...
                *_bc_operator = operator.map(|s| s.into());
                *_bc_count = count.map(|v| v as u32);
                *_bc_card_type = card_type.map(decode_cond_card_type);
        ...
        66 => {
            ...
            let count = read_u16_opt(cursor);
            let target = read_player_opt(cursor);
            let resource_type = read_resource_opt(cursor);
            let card_type = read_u8_opt(cursor);
            let group_names = read_str(cursor);
            let mut c = default_condition_comparison();
            if let Condition::Comparison {
                location: ref mut _bc_location,
                aggregate: ref mut _bc_aggregate,
                operator: ref mut _bc_operator,
                count: ref mut _bc_count,
                target: ref mut _bc_target,
                resource_type: ref mut _bc_resource_type,
                card_type: ref mut _bc_card_type,
                group_names: ref mut _bc_group_names,
                ..
            } = &mut c
            {
                ...
                *_bc_count = count.map(|v| v as u32);
                *_bc_target = target.map(|s| s.into());
                *_bc_resource_type = resource_type.map(|s| s.into());
                *_bc_card_type = card_type.map(decode_cond_card_type);
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
            }
            c
        ...
        68 => {
            let _ = read_u8(cursor);
            let movement = read_str(cursor);
            let location = read_zone_opt(cursor);
            let target = read_player_opt(cursor);
            let cost_limit = read_u16_opt(cursor);
            let cost_limit_operator = read_operator_opt(cursor);
            let baton_touch_trigger = read_bool_opt(cursor);
            let min_baton_touch_count = read_u8_opt(cursor);
            let exclude_self = read_bool_opt(cursor);
            let group_names = read_str_list(cursor);
            let card_type = read_card_type_opt(cursor);
            let characters = read_str_list(cursor);
            let baton_touch_source = read_zone_opt(cursor);
            let comparison_type = read_str(cursor);
            let operator = read_operator_opt(cursor);
            let self_effect_only = read_bool_opt(cursor);
            let energy_placed = read_bool_opt(cursor);
            let area_direction = read_zone_opt(cursor);
            let self_target = read_bool_opt(cursor);
            let source = read_zone_opt(cursor);
            let destination = read_zone_opt(cursor);
            let from_state = read_state_opt(cursor);
            let to_state = read_state_opt(cursor);
            let mut c = default_condition_movement();
            if let Condition::Movement {
                movement: ref mut _bc_movement,
                location: ref mut _bc_location,
                target: ref mut _bc_target,
                cost_limit: ref mut _bc_cost_limit,
                cost_limit_operator: ref mut _bc_cost_limit_operator,
                baton_touch_trigger: ref mut _bc_baton_touch_trigger,
                min_baton_touch_count: ref mut _bc_min_baton_touch_count,
                exclude_self: ref mut _bc_exclude_self,
                group_names: ref mut _bc_group_names,
                characters: ref mut _bc_characters,
                baton_touch_source: ref mut _bc_baton_touch_source,
                operator: ref mut _bc_operator,
                self_effect_only: ref mut _bc_self_effect_only,
                energy_placed: ref mut _bc_energy_placed,
                area_direction: ref mut _bc_area_direction,
                self_target: ref mut _bc_self_target,
                source: ref mut _bc_source,
                destination: ref mut _bc_destination,
                ..
            ...
            {
                *_bc_movement = movement.map(|s| s.into());
                *_bc_location = location.map(|s| s.into());
                *_bc_target = target.map(|s| s.into());
                *_bc_cost_limit = cost_limit.map(|v| v as u32);
                *_bc_cost_limit_operator = cost_limit_operator.map(decode_operator_from_str);
                *_bc_baton_touch_trigger = baton_touch_trigger;
                *_bc_min_baton_touch_count = min_baton_touch_count.map(|v| v as u32);
                *_bc_exclude_self = exclude_self;
                *_bc_group_names = if group_names.is_empty() {
                    None
                } else {
                ...
                *_bc_baton_touch_source = baton_touch_source.map(|s| s.into());
                *_bc_operator = operator.map(|s| s.into());
                *_bc_self_effect_only = self_effect_only;
                *_bc_energy_placed = energy_placed;
                *_bc_area_direction = area_direction.map(|s| s.into());
                *_bc_self_target = self_target;
                *_bc_source = source.map(|s| s.into());
                *_bc_destination = destination.map(|s| s.into());
            }
        ...
        84 => {
            let _ = read_u8(cursor);
            let no_excess_heart = read_bool_opt(cursor);
            let mut c = default_condition_opponentLiveSuccess();
            if let Condition::OpponentLiveSuccess {
                no_excess_heart: ref mut _bc_no_excess_heart,
                ..
            } = &mut c
            {
                *_bc_no_excess_heart = no_excess_heart;
            }
            c
        }
        73 => {
        ...
        71 => {
            ...
            let state = read_state_opt(cursor);
            let operator = read_operator_opt(cursor);
            let value = read_bool_opt(cursor);
            let mut c = default_condition_state();
            if let Condition::State {
                state: ref mut _bc_state,
                operator: ref mut _bc_operator,
...
fn decode_effect_kind(op: Opcode, cursor: &mut &[u8]) -> Option<Box<EffectKind>> {
    match op {
        ...
        Opcode::ChangeState => {
            ...
            let mut ek = default_changeState();
            if let EffectKind::ChangeState { action_by: ref mut _bc_action_by, all: ref mut _bc_all, blade_limit: ref mut _bc_blade_limit, blade_limit_operator: ref mut _bc_blade_limit_operator, card_type: ref mut _bc_card_type, cost_from_revealed: ref mut _bc_cost_from_revealed, cost_limit: ref mut _bc_cost_limit, cost_limit_operator: ref mut _bc_cost_limit_operator, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, location: ref mut _bc_location, optional: ref mut _bc_optional, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, self_cost: ref mut _bc_self_cost, self_target: ref mut _bc_self_target, source: ref mut _bc_source, state: ref mut _bc_state, state_change: ref mut _bc_state_change, target: ref mut _bc_target, .. } = &mut ek {
        ...
        Opcode::ChooseTargetPlayer => {
            ...
            let optional = read_u8(cursor) != 0;
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget { optional: ref mut _bc_optional, .. } = &mut ek {
                *_bc_optional = Some(optional);
            }
            Some(Box::new(ek))
        }
        Opcode::ConditionalAlternative => {
            ...
            let position = decode_zone(read_u8(cursor));
            let mut ek = default_compoundEffect();
            if let EffectKind::CompoundEffect { group_names: ref mut _bc_group_names, .. } = &mut ek {
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
            }
            Some(Box::new(ek))
        }
        ...
        Opcode::DrawCard => {
            ...
            let trigger_type = read_str(cursor);
            let mut ek = default_drawCards();
            if let EffectKind::DrawCards { action_by: ref mut _bc_action_by, card_type: ref mut _bc_card_type, target_count: ref mut _bc_target_count, destination: ref mut _bc_destination, exclude_self: ref mut _bc_exclude_self, heart_colors: ref mut _bc_heart_colors, location: ref mut _bc_location, original_value: ref mut _bc_original_value, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, source: ref mut _bc_source, state: ref mut _bc_state, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_action_by = Some(action_by.into());
                *_bc_card_type = Some(card_type.into());
        ...
        Opcode::DrawUntilCount => {
            ...
            let target_count = read_u8(cursor);
            let mut ek = default_drawCards();
            if let EffectKind::DrawCards { target_count: ref mut _bc_target_count, destination: ref mut _bc_destination, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_target_count = Some(count as u32);
                *_bc_destination = Some(destination.into());
                *_bc_source = Some(source.into());
                *_bc_target = Some(target.into());
        ...
        Opcode::GainAbilityFromSource => {
            ...
            let mut ek = default_abilityOp();
            if let EffectKind::AbilityOp { card_type: ref mut _bc_card_type, group_names: ref mut _bc_group_names, location: ref mut _bc_location, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
                *_bc_location = Some(source_location.into());
            }
            Some(Box::new(ek))
        }
        Opcode::GainResource => {
            ...
            let trigger_type = read_str(cursor);
            let mut ek = default_gainResource();
            if let EffectKind::GainResource { action_by: ref mut _bc_action_by, all: ref mut _bc_all, card_property: ref mut _bc_card_property, card_type: ref mut _bc_card_type, cost_limit: ref mut _bc_cost_limit, cost_limit_operator: ref mut _bc_cost_limit_operator, value: ref mut _bc_value, duration: ref mut _bc_duration, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, heart_color: ref mut _bc_heart_color, heart_colors: ref mut _bc_heart_colors, location: ref mut _bc_location, repeat_limit: ref mut _bc_repeat_limit, negation: ref mut _bc_negation, original_value: ref mut _bc_original_value, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, resource: ref mut _bc_resource, self_target: ref mut _bc_self_target, sign: ref mut _bc_sign, .. } = &mut ek {
                *_bc_action_by = Some(action_by.into());
                *_bc_all = Some(all);
                *_bc_card_property = card_property.map(|s| s.into());
                *_bc_card_type = Some(card_type.into());
                ...
                *_bc_heart_colors = Box::new(vec![heart_colors.to_string()]);
                *_bc_location = Some(location.into());
                *_bc_repeat_limit = Some(max_repeats as u32);
        ...
        Opcode::InvalidateAbility => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_abilityOp();
            if let EffectKind::AbilityOp { card_type: ref mut _bc_card_type, duration: ref mut _bc_duration, group_names: ref mut _bc_group_names, self_target: ref mut _bc_self_target, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_duration = Some(duration.into());
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
                *_bc_self_target = Some(self_target);
        ...
        Opcode::LookAt => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_lookReveal();
            if let EffectKind::LookReveal { card_type: ref mut _bc_card_type, location: ref mut _bc_location, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_location = Some(location.into());
                *_bc_per_unit = Some(per_unit);
                *_bc_per_unit_count = Some(per_unit_count as u32);
        ...
        Opcode::ModifyRequiredHearts => {
            ...
            let value = read_u8(cursor);
            let mut ek = default_modifyHearts();
            if let EffectKind::ModifyHearts { card_type: ref mut _bc_card_type, value: ref mut _bc_value, duration: ref mut _bc_duration, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, heart_colors: ref mut _bc_heart_colors, location: ref mut _bc_location, repeat_limit: ref mut _bc_repeat_limit, operation: ref mut _bc_operation, original_value: ref mut _bc_original_value, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, self_target: ref mut _bc_self_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_value = Some(count as u32);
                *_bc_duration = duration.map(|s| s.into());
                *_bc_exclude_self = Some(exclude_self);
        ...
        Opcode::ModifyScore => {
            ...
            let value = read_u8(cursor);
            let mut ek = default_modifyScore();
            if let EffectKind::ModifyScore { card_type: ref mut _bc_card_type, duration: ref mut _bc_duration, group_names: ref mut _bc_group_names, location: ref mut _bc_location, repeat_limit: ref mut _bc_repeat_limit, operation: ref mut _bc_operation, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, self_target: ref mut _bc_self_target, source: ref mut _bc_source, target: ref mut _bc_target, value: ref mut _bc_value, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_duration = duration.map(|s| s.into());
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
                *_bc_location = Some(location.into());
        ...
        Opcode::ModifyYellCount => {
            ...
            let operation = read_str(cursor);
            let mut ek = default_modifyScore();
            if let EffectKind::ModifyScore { value: ref mut _bc_value, duration: ref mut _bc_duration, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, operation: ref mut _bc_operation, .. } = &mut ek {
                *_bc_value = Some(count as u32);
                *_bc_duration = Some(duration.into());
                *_bc_exclude_self = Some(exclude_self);
                *_bc_group_names = group_names.map(|s| Box::new(vec![s.to_string()]));
        ...
        Opcode::PayEnergy => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_gainResource();
            if let EffectKind::GainResource { value: ref mut _bc_value, location: ref mut _bc_location, optional: ref mut _bc_optional, .. } = &mut ek {
                *_bc_value = Some(count as u32);
                *_bc_location = Some(location.into());
                *_bc_optional = Some(optional);
            }
        ...
        Opcode::PerformYell => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_miscOp();
            if let EffectKind::MiscOp { value: ref mut _bc_value, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_value = Some(count as u32);
                *_bc_target = Some(target.into());
            }
            Some(Box::new(ek))
        ...
        Opcode::PositionChange => {
            ...
            let target_member = read_str(cursor);
            let mut ek = default_positionOp();
            if let EffectKind::PositionOp { card_type: ref mut _bc_card_type, destination: ref mut _bc_destination, exclude_position: ref mut _bc_exclude_position, exclude_self: ref mut _bc_exclude_self, optional: ref mut _bc_optional, source: ref mut _bc_source, source_position: ref mut _bc_source_position, target: ref mut _bc_target, target_member: ref mut _bc_target_member, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_destination = Some(destination.into());
        ...
        Opcode::ReYell => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_miscOp();
            if let EffectKind::MiscOp { target: ref mut _bc_target, .. } = &mut ek {
                *_bc_target = Some(target.into());
            }
            Some(Box::new(ek))
        }
        ...
        Opcode::Restriction => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_restrictionOp();
            if let EffectKind::RestrictionOp { card_type: ref mut _bc_card_type, delayed: ref mut _bc_delayed, duration: ref mut _bc_duration, phase: ref mut _bc_phase, restriction_type: ref mut _bc_restriction_type, self_target: ref mut _bc_self_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
        ...
        Opcode::Reveal => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_lookReveal();
            if let EffectKind::LookReveal { card_type: ref mut _bc_card_type, exclude_self: ref mut _bc_exclude_self, location: ref mut _bc_location, per_unit: ref mut _bc_per_unit, per_unit_count: ref mut _bc_per_unit_count, per_unit_type: ref mut _bc_per_unit_type, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_exclude_self = Some(exclude_self);
                *_bc_location = Some(location.into());
                *_bc_per_unit = Some(per_unit);
        ...
        Opcode::Select => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget { ability_filter_triggers: ref mut _bc_ability_filter_triggers, action_by: ref mut _bc_action_by, card_type: ref mut _bc_card_type, target_count: ref mut _bc_target_count, exclude_selected: ref mut _bc_exclude_selected, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, optional: ref mut _bc_optional, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
        ...
        Opcode::SelectCards => {
            ...
            let source = decode_zone(read_u8(cursor));
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget { any_number: ref mut _bc_any_number, card_type: ref mut _bc_card_type, target_count: ref mut _bc_target_count, destination: ref mut _bc_destination, discard_remaining: ref mut _bc_discard_remaining, exclude_self: ref mut _bc_exclude_self, group_names: ref mut _bc_group_names, optional: ref mut _bc_optional, original_value: ref mut _bc_original_value, per_group: ref mut _bc_per_group, per_group_count: ref mut _bc_per_group_count, reveal: ref mut _bc_reveal, source: ref mut _bc_source, .. } = &mut ek {
                *_bc_any_number = Some(any_number);
                *_bc_card_type = Some(card_type.into());
                *_bc_target_count = Some(count as u32);
                *_bc_destination = Some(destination.into());
        ...
        Opcode::SelectNumber => {
            let count = read_u8(cursor);
            let mut ek = default_selectTarget();
            if let EffectKind::SelectTarget { target_count: ref mut _bc_target_count, .. } = &mut ek {
                *_bc_target_count = Some(count as u32);
            }
            Some(Box::new(ek))
        }
        Opcode::SetBladeCount => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_miscOp();
            if let EffectKind::MiscOp { card_type: ref mut _bc_card_type, value: ref mut _bc_value, duration: ref mut _bc_duration, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_value = Some(count as u32);
                *_bc_duration = Some(duration.into());
                *_bc_target = Some(target.into());
        ...
        Opcode::SetCardIdentity => {
            ...
            let self_target = read_u8(cursor) != 0;
            let mut ek = default_changeState();
            if let EffectKind::ChangeState { all: ref mut _bc_all, all_regions: ref mut _bc_all_regions, group_names: ref mut _bc_group_names, self_target: ref mut _bc_self_target, .. } = &mut ek {
                *_bc_all = Some(all);
        ...
        Opcode::SetHeartType => {
            ...
            let target = decode_player(read_u8(cursor));
            let mut ek = default_miscOp();
            if let EffectKind::MiscOp { card_type: ref mut _bc_card_type, value: ref mut _bc_value, duration: ref mut _bc_duration, self_target: ref mut _bc_self_target, source: ref mut _bc_source, target: ref mut _bc_target, .. } = &mut ek {
                *_bc_card_type = Some(card_type.into());
                *_bc_value = Some(count as u32);
                *_bc_duration = Some(duration.into());
                *_bc_self_target = Some(self_target);
